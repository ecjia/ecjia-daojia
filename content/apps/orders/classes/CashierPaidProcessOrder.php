...
class CashierPaidProcessOrder
{
    ...
    public static function processOrderDefaultship($orderinfo)
    {
        ...
        if (is_ecjia_error($res)) {
            RC_Logger::getLogger('error')->info(sprintf(__('收银台订单发货后更新商品销量失败【订单id|%s】：' . $res->get_error_message(), 'orders'), $orderinfo['order_id']));
        }
    }
    ...
    {
        $result = RC_Api::api('orders', 'order_operate', array('order_id' => $order_info['order_id'], 'order_sn' => '', 'operation' => 'prepare', 'note' => array('action_note' => __('收银台配货', 'orders'))));
        if (is_ecjia_error($result)) {
            RC_Logger::getLogger('error')->info(sprintf(__('收银台订单配货【订单id|%s】：' . $result->get_error_message(), 'orders'), $order_info['order_id']));
        }
    }
    ...
    {
        $result = RC_Api::api('orders', 'order_operate', array('order_id' => $order_info['order_id'], 'order_sn' => '', 'operation' => 'split', 'note' => array('action_note' => __('收银台生成发货单', 'orders'))));
        if (is_ecjia_error($result)) {
            RC_Logger::getLogger('error')->info(sprintf(__('收银台订单分单【订单id|%s】：' . $result->get_error_message(), 'orders'), $order_info['order_id']));
        } else {
            /*订单状态日志记录*/
    ...
    private static function _ship($order_info)
    {
        ...
        if (is_ecjia_error($result)) {
            RC_Logger::getLogger('error')->info(sprintf(__('收银台订单发货【订单id|%s】：' . $result->get_error_message(), 'orders'), $order_info['order_id']));
        } else {
            /*订单状态日志记录*/
    ...
    private static function _affirmReceived($order_info)
    {
        ...
        /* 记录log */
        order_action($order_info['order_sn'], OS_SPLITED, SS_RECEIVED, PS_PAYED, __('收银台确认收货', 'orders'));
    }
