...
class OrderMerchantClosed
{
     ...
     * @return bool
     */
    public static function CloseOrderToMerchantAgree($order = array(), $staff_id = 0, $cancel_note = '')
    {
        /**
        ...
        self::_updateWithLogs($order, $refund_id, $cancel_note, $staff_id);
        //订单被拒，发送短信通知
    ...
    /**
     * 关闭订单（已支付，待发货的）；默认退款，流程至平台打款完成
     * @param array $order 订单信息
     * @return bool
     */
    public static function CloseOrderToRefundFinished($order = array(), $staff_id = 0, $cancel_note = '')
    {
    	/**
    	 * 1、生成退款申请单_produceRefundOrder();
    	 * 2、更新订单状态&订单操作记录&订单状态记录；及更新售后申请状态记录; _updateWithLogs();
    	...
    	$res = self::CloseOrderToMerchantAgree($order, $staff_id = 0, $cancel_note = '');
...
//     	if ($res) {
//     		$refund_id = $res;
//     		$result = self::platformPayrecordUpdate($refund_id, $staff_id);
...
//     	}
    ...
    private static function _produceRefundOrder($order)
    {
        ...
        $auto_refuse    = $reasons['merchant_refuse'];
     ...
     * 更新订单状态&订单操作记录&订单状态记录；及更新售后申请状态记录;
     */
    private static function _updateWithLogs($order, $refund_id, $cancel_note = '', $staff_id = 0)
    {
        /* 订单状态为“退货” */
        ...
        $username = empty($staff_info['name']) ? '' : $staff_info['name'];
        
        /* 记录log */
        $action_note = empty($cancel_note) ? __('商家关闭订单自动退款', 'orders') : $cancel_note;
        order_refund::order_action($order['order_id'], OS_RETURNED, $order['shipping_status'], $order['pay_status'], $action_note, $username);

        //订单状态log记录
        $pra = array('order_status' => __('商家关闭订单', 'orders'), 'order_id' => $order['order_id'], 'message' => __('等待商家退款！', 'orders'));
        order_refund::order_status_log($pra);

        //售后申请状态记录
        $opt = array('status' => __('商家关闭订单', 'orders'), 'refund_id' => $refund_id, 'message' => __('等待商家退款！', 'orders'));
    ...
    private static function _merchantAgree($refund_id)
    {
        ...
        $data = array(
            ...
            'action_user_id'   => 0,
            'action_user_name' => __('商家关闭订单自动退款', 'orders'),
            'status'           => 1,
            'refund_status'    => $refund_status,
            'return_status'    => $refund_status,
            'action_note'      => __('商家关闭订单自动退款至余额', 'orders'),
     ...
     */
    private static function platformPayrecordUpdate($refund_id, $staff_id)
    {
    	$staff_info = RC_DB::table('staff_user')->where('user_id', $staff_id)->first();
    	...
    	if ($refund_order) {
    		...
    		if ($refund_order['refund_status'] == \Ecjia\App\Refund\Enums\RefundPayEnum::PAY_TRANSFERED) {
    			return new \ecjia_error('order_refunded', __('该订单已进行打款处理，请勿重复操作！', 'orders'));
    		}
    		$result = (new \Ecjia\App\Payment\Refund\RefundManager($refund_order['order_sn'], null, null))->refundToBalance($real_back_money_total, $staff_info['name']);
    		
    		if (is_ecjia_error($result)) {    			
    			return new \ecjia_error('refund_error', __($result->get_error_message(), 'orders'));
    		}
    		
    		//更新打款表
    		(new \Ecjia\App\Refund\Models\RefundPayRecordModel)->updateRefundPayrecord($payrecord_info['id'], 'surplus', $back_content, $staff_id, $staff_info['name']);
    		
    		//更新refund_order_action表打款操作人信息
    		RC_DB::table('refund_order_action')->where('refund_id', $refund_id)->where('refund_status', \Ecjia\App\Refund\Enums\RefundPayEnum::PAY_TRANSFERED)->update(array('action_user_type' => 'admin', 'action_user_id' => $staff_id, 'action_user_name' => $staff_info['name']));
