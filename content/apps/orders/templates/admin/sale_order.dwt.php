...
<div>
	<h3 class="heading">
		...
		<!-- {if $action_link} -->
			<a class="btn plus_or_reply"  id="sticky_a" href="{$action_link.href}&start_date={$start_date}&end_date={$end_date}&sort_by={$filter.sort_by}&sort_order={$filter.sort_order}{if $smarty.get.merchant_keywords}&merchant_keywords={$smarty.get.merchant_keywords}{/if}{if $smarty.get.store_id}&store_id={$smarty.get.store_id}{/if}"><i class="fontello-icon-download"></i>{$action_link.text}</a>
		<!-- {/if} -->
		<!-- {if $smarty.get.store_id} -->
			<a class="btn plus_or_reply" href='{RC_Uri::url("orders/admin_sale_order/init", "{$url_args}")}'><i class="fontello-icon-reply"></i>{t domain="orders"}返回全部{/t}</a>
		<!-- {/if} -->
	</h3>
</div>
...
<div class="row-fluid">
	<div class="choose_list f_r">
		<form class="f_r" action="{$search_action}"  method="post" name="theForm">
			...
			<input class="btn screen-btn" type="submit" name="submit" value='{t domain="orders"}搜索{/t}'>
		</form>
	</div>
</div>
	...
	<table class="table table-striped" id="smpl_tbl">
		...
		<tbody>
			...
			<tr>
				...
				<td>
					<a href='{RC_Uri::url("orders/admin_sale_order/init", "store_id={$list.store_id}{$url_args}")}'>{$list.merchants_name}<a href='{RC_Uri::url("store/admin/preview", "store_id={$list.store_id}")}' title='{t domain="orders"}查看商家资料{/t}' target="_blank"><i class="fontello-icon-info-circled"></i></a></a>
				</td>
				<td>{$list.goods_sn}</td>
			...
			<!-- {foreachelse} -->
	    	<tr><td class="dataTables_empty" colspan="7">{t domain="orders"}没有找到任何记录{/t}</td></tr>
	  		<!-- {/foreach} -->
		</tbody>
	</table>
