				...
				<table class="table order-goods-select table-striped">
					...
					<tbody>
						...
						<tr class='edit_order_list'>
							...
							<td>
								<a class="ajaxremove no-underline" data-toggle="ajaxremove" data-msg='{t domain="orders" 1="{$goods.goods_name}"}您确定要删除订单商品[ %1 ]吗？{/t}' href='{url path="orders/merchant/process" args="func=drop_order_goods&rec_id={$goods.rec_id}&step_act={$step_act}&order_id={$order_id}"}' title='{t domain="orders"}移除{/t}'><button class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i></button></a>
							</td>
						</tr>
						<!-- {/foreach} -->
						<tr>
...
<!-- 								<button class="btn btn-info" type="submit" name="edit_goods">{t domain="orders"}更新商品{/t}</button> -->
<!-- 								{/if} -->
                        			<p class="ecjiaf-tac m_t15">
                        				<button class="btn btn-info" type="submit" name="finish">{t domain="orders"}更新商品{/t}</button>&nbsp;&nbsp;&nbsp;
                        				<input name="finish" type="hidden" value='{t domain="orders"}确定{/t}' />
                        				<a class="cancel_order" data-href='{url path="orders/merchant/process" args="func=cancel_order&order_id={$order_id}&step_act={$step_act}"}'><button class="btn btn-info" type="button">{t domain="orders"}取消{/t}</button></a>
                        			</p>
...
<div class="row">
	<div class="col-lg-12 panel-heading form-inline">
		<span>{t domain="orders"}按商品编号或商品名称或商品货号搜索：{/t}</span>
		<input class="form-control" type="text" name="keyword" placeholder='{t domain="orders"}请输入关键字{/t}' />
	...
	<div class="ms-container ms-container-nobg" id="ms-custom-navigation">
		<div class="ms-selectable ms-not-selectable" id="goodslist"  data-change-url='{url path="orders/merchant/goods_json"}'>
			<div class="search-header">
				<input class="form-control" id="ms-search" type="text" placeholder='{t domain="orders"}筛选搜索到的商品信息{/t}' autocomplete="off">
			</div>
...
<!-- 									<dd>{t domain="orders"}品牌{/t}：<span id="goods_brand"></span></dd> -->
						...
						<div class="ecjiaf-dn goods_info row-fluid">
							<div class="form-group">
								<label class="control-label col-lg-2">{t domain="orders"}商品价格：{/t}</label>
						...
						<div class="ecjiaf-dn goods_info row-fluid">
							<div class="form-group">
								<label class="control-label col-lg-2">{t domain="orders"}商品数量：{/t}</label>
...
<form class="form-horizontal" name="consigneeForm" action='{url path="orders/merchant/step_post" args="step={$step}&order_id={$order_id}&step_act={$step_act}"}' method="post" >
				...
				<table class="table table-striped table-hover table-hide-edit">
					...
					<tbody>
						...
						<tr class="{if $val.default_address}info{/if}">
							...
							<td>
                                {t domain="orders"}电话：{/t}{$val.tel}<br/>
      		...
      		<section class="panel">
          		<div class="panel-body">
					<div class="row-fluid m_t20 {if $address_list && $order.consignee eq ''}ecjiaf-dn{/if}">
						...
						<!--{if $exist_real_goods} -->
						<div class="order-step-formgroup form-group">
							<label class="control-label col-lg-2">{t domain="orders"}详细地址：{/t}</label>
							<div class="col-lg-6 form-inline" >
						...
						<div class="form-group order-step">
                            <label class="control-label col-lg-2">{t domain="orders"}省份：{/t}</label>
						...
						<div class="order-step-formgroup form-group">
							<label class="control-label col-lg-2">{t domain="orders"}邮编：{/t}</label>
							<div class="col-lg-6 form-inline" >
	...
	<p class="ecjiaf-tac m_t15">
		...
		<button class="btn btn-info" type="button">{t domain="orders"}取消{/t}</button></a>
		<input name="finish" type="hidden" value='{t domain="orders"}确定{/t}' />
	</p>
</form>
				...
				<div class="panel-heading">
                    <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne">
                        <h4 class="panel-title">
                            <strong>{t domain="orders"}发票相关{/t}</strong>
                        </h4>
                    </a>
                </div>
				<div class="accordion-body in collapse " id="collapseOne">
					<div class="form-group first-form-group">
						<label class="control-label col-lg-2">{t domain="orders"}发票类型：{/t}</label>
						<div class="col-lg-8">
							<input name="inv_type" class="form-control" type="text" id="inv_type" value="{$order.inv_type}"/>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-lg-2">{t domain="orders"}发票抬头：{/t}</label>
						<div class="col-lg-8">
							<input name="inv_payee" class="form-control" value="{$order.inv_payee}" type="text" />
							<span class="help-block">{t domain="orders"}发票抬头及发票识别码，请用英文逗号（“,”）隔开，例：抬头,识别码。如没有英文逗号，则默认为发票抬头。{/t}</span>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-lg-2">{t domain="orders"}发票内容：{/t}</label>
						<div class="col-lg-8">
			...
			<div id="accordion2" class="panel panel-default order-step-accordion">
				<div class="panel-heading">
                    <a data-toggle="collapse" data-parent="#accordion" href="#collapseTwo">
                        <h4 class="panel-title">
                            <strong>{t domain="orders"}留言/备注{/t}</strong>
                        </h4>
                    </a>
                </div>
				<div class="accordion-body in collapse " id="collapseTwo">
					<div class="form-group first-form-group">
						<label class="control-label col-lg-2">{t domain="orders"}客户给商家的留言：{/t}</label>
						<div class="col-lg-8">
							<textarea name="postscript" class="form-control action_note" cols="60" rows="3">{$order.postscript}</textarea>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-lg-2">{t domain="orders"}缺货处理：{/t}</label>
						<div class="col-lg-8">
							<textarea name="how_oos" class="form-control action_note" cols="60" rows="3">{$order.how_oos}</textarea>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-lg-2">{t domain="orders"}商家给客户的留言：{/t}</label>
						<div class="col-lg-8">
		...
		<form name="moneyForm" action='{url path="orders/merchant/step_post" args="step={$step}&order_id={$order_id}&step_act={$step_act}"}' method="post">
			<div id="accordion2" class="panel panel-default order-step-accordion">
				<div class="panel-heading">
                    <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne">
                        <h4 class="panel-title">
                            <strong>{t domain="orders"}订单信息{/t}</strong>
                        </h4>
                    </a>
                </div>
				<div class="accordion-body in collapse " id="collapseOne">
...
<form class="form-horizontal" name="invoiceForm" action='{url path="orders/merchant/step_post" args="step={$step}&order_id={$order_id}&step_act={$step_act}"}' method="post">
	<div class="form-group">
		<label class="control-label col-lg-2"><strong>{t domain="orders"}运单编号：{/t}</strong></label>
		<div class="col-lg-6">
			<input class="w250 form-control" name="invoice_no" type="text" value="{$order.invoice_no}" size="30"/>
			<span class="help-block">{t domain="orders"}多个发货单号，请用英文逗号（“,”）隔开。{/t}</span>
		</div>
	</div>
	...
	<div class="row-fluid">
		<table class="table table-striped">
			...
			<tbody>
				...
				<!--{foreachelse}-->
				<tr>
					<td class="no-records" colspan="3">{t domain="orders"}没有找到任何记录{/t}</td>
				</tr>
				<!-- {/foreach} -->
			</tbody>
		</table>
	...
	<p align="center">
		...
		<a class="data-pjax" href='{url path="orders/merchant/info" args="order_id={$order_id}"}'><button class="btn btn-info" type="button">{t domain="orders"}取消{/t}</button></a>
	</p>
</form>
</div>
</div>
