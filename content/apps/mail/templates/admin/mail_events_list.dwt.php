	...
	<div class="span12">
        <table class="table table-striped smpl_tbl dataTable table-hide-edit" id="plugin-table">
            ...
            <tr>
                ...
                <td>
                    <a class="change_status" style="cursor: pointer;"  data-msg='{if $list.status eq 'open'}{t domain="mail"}您确定要关闭该邮件事件吗？{/t}{else}{t domain="mail"}您确定要开启该邮件事件吗？{/t}{/if}' data-href='{if $list.status eq "open"}{url path="mail/admin_events/close" args="code={$list.code}&id={$list.id}"}{else}{url path="mail/admin_events/open" args="code={$list.code}&id={$list.id}"}{/if}' >
