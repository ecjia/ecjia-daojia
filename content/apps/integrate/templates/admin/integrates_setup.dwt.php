...
<!--{else}-->
<div class="form-div">
    <div class="alert alert-info" >{lang key='integrate::integrate.UCenter_integration'}</div>
</div>
<!--{/if}-->
<div>
	<h3 class="heading">
		...
		<div id="form3">
			<form class="form-horizontal" id="form-privilege" action="{$form_action}" method="post" name="setupForm">
				<fieldset>
					...
					<div class="control-group ">
						<label class="control-label">{lang key='integrate::integrate.lable_db_name'}</label>
						<div class="controls users">
							<input class="w350" type="text" name="cfg[db_name]" value="{$cfg.db_name}" /><i class="input-must">{lang key='system::system.require_field'}</i>
						</div>
					</div>
					<div class="control-group ">
						<label class="control-label">{lang key='integrate::integrate.lable_db_chartset'}</label>
						<div class="controls users">
							<select class="w350" name="cfg[db_charset]">{html_options options=$set_list selected=$cfg.db_charset}</select>
							<span class="help-block"> {lang key='integrate::integrate.notice_latin1'}</span>
						</div>
					</div>
					<div class="control-group ">
						<label class="control-label">{lang key='integrate::integrate.lable_is_latin1'}：</label>
						<div class="controls users">
							<input type="radio" name="cfg[is_latin1]" value="1" {if $cfg.is_latin1}checked="checked"{/if} />{lang key='integrate::integrate.yes'}
							<input type="radio" name="cfg[is_latin1]" value="0" {if !$cfg.is_latin1}checked="checked"{/if} />{lang key='integrate::integrate.no'}
						</div>
					</div>
					<div class="control-group ">
						<label class="control-label">{lang key='integrate::integrate.lable_prefix'}</label>
						<div class="controls users">
							<input class="w350" type="text" name="cfg[prefix]" value="{$cfg.prefix}" />
						</div>
					</div> 
					<div class="control-group ">
						<label class="control-label">{lang key='integrate::integrate.lable_url'}</label>
						<div class="controls users">
					...
					<div class="control-group ">
						<label class="control-label">{lang key='integrate::integrate.cookie_prefix'}</label>
						<div class="controls users">
					...
					<div class="control-group ">
						<label class="control-label">{lang key='integrate::integrate.cookie_salt'}</label>
						<div class="controls users">
					...
					<div class="control-group">
						<div class="controls">
							<input type="submit" value="{lang key='integrate::integrate.button_save_config'}" class="btn btn-gebo" />
