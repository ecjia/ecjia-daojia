...
<div class="row-fluid batch">
	...
	<form class="f_l" action="" name="searchForm">
		<div class="btn-group f_l m_r5">
			...
			<ul class="dropdown-menu batch-move">
				<li><a class="batch-trash-btn" data-toggle="ecjiabatch" data-idclass=".checkbox:checked" data-url='{RC_Uri::url("goods/admin_parameter_attribute/batch", "cat_id={$cat_id}")}' data-msg="{t domain='goods'}您确定要删除选中的商品参数吗？{/t}" data-noselectmsg="{t domain='goods'}您没有选择需要删除的参数{/t}" href="javascript:;"><i class="fontello-icon-trash"></i>{t domain="goods"}批量删除{/t}</a></li>
			</ul>
		</div>
	</form>
	...
	<div class="choose_list f_r">
		...
		<select name="goods_type" data-url="{url path='goods/admin_parameter_attribute/init' args='cat_id='}">
			{$goods_type_list}
		</select>
	</div>
</div>
			...
			<tbody>
				...
				<tr>
					...
					<td class="first-cell" nowrap="true" valign="top">
						<span class="cursor_pointer" data-trigger="editable" data-url="{RC_Uri::url('goods/admin_parameter_attribute/edit_attr_name')}" data-name="edit_attr_name" data-pk="{$attr.attr_id}" data-title="{t domain='goods'}参数名称不能为空{/t}">
							{$attr.attr_name}
						</span>
					</td>
					<td nowrap="true" valign="top">
					...
					<td align="right" nowrap="true" valign="top">
						<span class="cursor_pointer" data-trigger="editable" data-url="{RC_Uri::url('goods/admin_parameter_attribute/edit_sort_order')}" data-name="edit_sort_order" data-pjax-url='{url path="goods/admin_parameter_attribute/init" args="cat_id={$smarty.get.cat_id}"}' data-pk="{$attr.attr_id}" data-title="{t domain='goods'}请输入排序号{/t}">{$attr.sort_order}</span>
					</td>
					<td align="center" nowrap="true" valign="top">
						<a class="data-pjax" href='{RC_Uri::url("goods/admin_parameter_attribute/edit", "attr_id={$attr.attr_id}")}' title="{t domain='goods'}编辑{/t}"><i class="fontello-icon-edit"></i></a>
						<a class="ajaxremove" data-toggle="ajaxremove" data-msg="{t domain='goods'}您确实要删除该参数吗？{/t}" href='{RC_Uri::url("goods/admin_parameter_attribute/remove", "id={$attr.attr_id}")}' title="{t domain='goods'}删除{/t}"><i class="fontello-icon-trash"></i></a>
					</td>
				</tr>
				<!-- {foreachelse} -->
				<tr>
