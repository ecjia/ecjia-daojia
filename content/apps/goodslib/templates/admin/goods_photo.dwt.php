...
<div class="row-fluid goods-photo-list{if !$img_list} hide{/if}">
		...
		<div class="wmk_grid ecj-wookmark wookmark_list">
			<ul class="wookmark-goods-photo move-mod nomove">
				...
				<p>
					...
					<a href="javascript:;" title="{lang key='goods::goods.save'}" data-toggle="sort-ok" data-imgid="{$img.img_id}" data-saveurl="{url path='goodslib/admin_gallery/update_image_desc'}" style="display:none;"><i class="fontello-icon-ok"></i></a>
					<a class="ajaxremove" data-imgid="{$img.img_id}" data-toggle="ajaxremove" data-msg="{lang key='goods::goods.drop_photo_confirm'}" href='{url path="goodslib/admin_gallery/drop_image" args="img_id={$img.img_id}&goods_id={$smarty.get.goods_id}"}' title="{lang key='system::system.remove'}"><i class="icon-trash"></i></a>
	...
	<a class="btn btn-info save-sort" data-sorturl="{url path='goodslib/admin_gallery/sort_image'}">{lang key='goods::goods.save_sort'}</a>
</div>
<div class="row-fluid">
	{if $step}
	<fieldset class="t_c">
		<button class="btn btn-gebo next_step" data-url="{$url}">{lang key='goods::goods.next_step'}</button>
		<button class="btn btn-gebo complete" data-url='{if $code}{url path="goodslib/admin/init" args="extension_code={$code}"}{else}{url path="goodslib/admin/init"}{/if}'>{lang key='goods::goods.complete'}</button>
