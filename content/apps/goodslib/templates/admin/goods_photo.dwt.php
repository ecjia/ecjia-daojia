...
<div class="row-fluid goods-photo-list{if !$img_list} hide{/if}">
	<div class="span12">
		<h3 class="heading m_b10">{t domain="goodslib"}商品相册{/t}<small>{t domain="goodslib"}（编辑、排序、删除）{/t}</small></h3>
		<div class="m_b20">
...
<div class="row-fluid">
	{if $step}
	<fieldset class="t_c">
		<button class="btn btn-gebo next_step" data-url="{$url}">{t domain="goodslib"}下一步{/t}</button>
		<button class="btn btn-gebo complete" data-url='{if $code}{url path="goodslib/admin/init" args="extension_code={$code}"}{else}{url path="goodslib/admin/init"}{/if}'>{t domain="goodslib"}完成{/t}</button>
