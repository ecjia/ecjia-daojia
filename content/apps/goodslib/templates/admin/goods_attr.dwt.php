			...
			<form class="form-horizontal" action="{$form_action}" method="post" name="theForm">
				<fieldset>
					<div class="control-group formSep">
						<label class="control-label">{t domain="goodslib"}商品规格：{/t}</label>
						<div class="controls">
							<select name="goods_type" autocomplete="off" data-toggle="get_attr_list" data-url='{url path="goodslib/admin/get_attr" args="goods_id={$goods_id}"}'>
								<option value="0">{t domain="goodslib"}请选择商品规格{/t}</option>
								<!-- {$goods_type_list} -->
							</select>
							<br/>
							<br/>
							<span class="help-block">{t domain="goodslib"}请选择商品的所属规格，进而完善此商品的属性{/t}</span>
						</div>
					</div>
				...
				<fieldset class="t_c">
					{if $step}
					<button class="btn btn-gebo" type="submit">{t domain="goodslib"}下一步{/t}</button>
