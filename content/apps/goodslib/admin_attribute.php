...
class admin_attribute extends ecjia_admin {
	...
	public function init() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('商品属性', 'goodslib')));
		ecjia_screen::get_current_screen()->add_help_tab(array(
			'id'		=> 'overview',
			'title'		=> __('概述', 'goodslib'),
			'content'	=> '<p>' . __('欢迎访问ECJia智能后台商品属性列表页面，系统中所有的商品属性都会显示在此列表中。', 'goodslib'). '</p>'
		));
		
		ecjia_screen::get_current_screen()->set_help_sidebar(
			'<p><strong>' . __('更多信息：', 'goodslib') . '</strong></p>' .
			'<p>' . __('<a href="https://ecjia.com/wiki/帮助:ECJia智能后台:商品类型#.E5.B1.9E.E6.80.A7.E5.88.97.E8.A1.A8" target="_blank">关于商品属性列表帮助文档</a>', 'goodslib') . '</p>'
		);
		...
		$this->assign('ur_here', __('商品属性', 'goodslib'));
		$this->assign('action_link', array('href' => RC_Uri::url('goodslib/admin_attribute/add', array('cat_id' => $goods_type)), 'text' => __('添加属性', 'goodslib')));
		$this->assign('action_link2', array('text' => __('商品规格列表', 'goodslib'), 'href' => RC_Uri::url('goodslib/admin_goods_spec/init')));
		
		$this->assign('goods_type_list', goods_type_list($goods_type, false));
		$this->assign('attr_list', $attr_list);
		$this->assign('cat_id', $goods_type);
		$this->assign('form_action', RC_Uri::url('goodslib/admin_attribute/batch'));
	...
	public function add() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('商品属性', 'goodslib'), RC_Uri::url('goodslib/admin_attribute/init', 'cat_id='.$cat_id)));
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('添加属性', 'goodslib')));
	
		ecjia_screen::get_current_screen()->add_help_tab(array(
			'id'		=> 'overview',
			'title'		=> __('概述', 'goodslib'),
			'content'	=> '<p>' . __('欢迎访问ECJia智能后台添加商品属性页面，可以在此页面添加商品属性信息。', 'goodslib') . '</p>'
		));
		
		ecjia_screen::get_current_screen()->set_help_sidebar(
			'<p><strong>' . __('更多信息：', 'goodslib') . '</strong></p>' .
			'<p>' . __('<a href="https://ecjia.com/wiki/帮助:ECJia智能后台:商品类型#.E6.B7.BB.E5.8A.A0.E5.B1.9E.E6.80.A7" target="_blank">关于添加商品属性帮助文档</a>', 'goodslib') . '</p>'
		);
		...
		$this->assign('form_action', RC_Uri::url('goodslib/admin_attribute/insert'));
	...
	public function insert() {
		...
		if (empty($cat_id)) {
			return $this->showmessage(__('商品类型不能为空', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		$count = RC_DB::table('attribute')->where('attr_name', trim($_POST['attr_name']))->where('cat_id', $cat_id)->count();
		if ($count) {
			return $this->showmessage(__('该属性名称已存在，请您换一个名称。', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		/* 取得属性信息 */
		$attr = array(
			'cat_id'			=> $cat_id,
		...
		if ($attr_id) {
			...
			$links = array(
				array('text' => __('返回属性列表', 'goodslib'), 'href' => RC_Uri::url('goodslib/admin_attribute/init', array('cat_id' => $cat_id))),
				array('text' => __('添加下一个属性', 'goodslib'), 'href' => RC_Uri::url('goodslib/admin_attribute/add', array('cat_id' => $cat_id))),
			);
			return $this->showmessage(sprintf(__('添加属性 [%s] 成功。', 'goodslib'), $attr['attr_name']), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goodslib/admin_attribute/edit', array('attr_id' => $attr_id)), 'links' => $links));
		}
	}
	...
	public function edit() {
		...
		ecjia_screen::get_current_screen()->add_help_tab(array(
			'id'		=> 'overview',
			'title'		=> __('概述', 'goodslib'),
			'content'	=> '<p>' . __('欢迎访问ECJia智能后台编辑商品属性页面，可以在此页面编辑商品属性信息。', 'goodslib') . '</p>'
		));
		
		ecjia_screen::get_current_screen()->set_help_sidebar(
			'<p><strong>' . __('更多信息：', 'goodslib') . '</strong></p>' .
			'<p>' . __('<a href="https://ecjia.com/wiki/帮助:ECJia智能后台:商品类型#.E7.BC.96.E8.BE.91.E5.B1.9E.E6.80.A7" target="_blank">关于编辑商品属性帮助文档</a>', 'goodslib') . '</p>'
		);
	...
	public function update() {
		...
		return $this->showmessage(sprintf(__('编辑属性 [%s] 成功。', 'goodslib'), $attr['attr_name']), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('links' => $links, 'pjaxurl' => RC_Uri::url('goodslib/admin_attribute/edit', array('attr_id' => $attr_id))));
	}
	...
	public function batch() {
		...
		if (isset($_POST['checkboxes'])) {
			...
			return $this->showmessage(sprintf(__('成功删除了 %d 条商品属性', 'goodslib'), $count), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goodslib/admin_attribute/init', array('cat_id' => $cat_id))));
		}
	}
	...
	public function edit_attr_name() {
		...
		if (!empty($val)) {
			...
			if (RC_DB::table('attribute')->where('attr_id', $id)->update($data)) {
				...
				return $this->showmessage(__('编辑成功', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('content' => $val));
			}
		} else {
			return $this->showmessage(__('请输入属性名称', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
	}
	...
	public function edit_sort_order() {
		...
		if (!is_numeric($val) || $val < 0 || strpos($val, '.') > 0) {
			return $this->showmessage(__('请输入大于0的整数', 'goodslib'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
