...
class goods_image_data {
	...
	public function update_goods($img_desc = null) {
	    ...
	    /* 如果有上传图片，删除原来的商品图 */
	    $row = RC_DB::table('goodslib')->where('goods_id', $this->goods_id)->select('goods_thumb','goods_img','original_img')->first();
...
// 	    goods_imageutils::deleteImage($this->file_path);
	    ...
	    if (!ecjia::config('retain_original_img') && !empty($goods_original)) {
	        RC_DB::table('goodslib')->where('goods_id', $this->goods_id)->update(array('original_img' => ''));
