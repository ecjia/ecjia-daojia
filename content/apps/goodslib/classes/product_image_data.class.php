...
class product_image_data {
	...
	public function update_goods($img_desc = null) {
	    ...
	    /* 如果有上传图片，删除原来的商品图 */
	    $row = RC_DB::table('goodslib_products')->where('product_id', $this->product_id)->first();
        ...
        RC_DB::table('goodslib_products')->where('product_id', $this->product_id)->update($data);
...
// 	    goods_imageutils::deleteImage($this->file_path);
	    ...
	    if (!ecjia::config('retain_original_img') && !empty($goods_original)) {
            RC_DB::table('goodslib_products')->where('product_id', $this->product_id)->update(array('product_original_img' => ''));
...
// 	    goods_imageutils::deleteImage($this->file_path);
	    ...
	    if (!ecjia::config('retain_original_img') && !empty($data['img_original'])) {
            RC_DB::table('goodslib_gallery')->where('product_id', $this->product_id)->update(array('img_original' => ''));
