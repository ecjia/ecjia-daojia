...
//			$( ".nav-list-ready ,.ms-selection .nav-list-content" ).disableSelection();
		...
		load_link_user_opt : function(data) {
			...
			if (data.content.length > 0) {
				for (var i = 0; i < data.content.length; i++) {
					...
					var opt = '<li class="ms-elem-selectable ' + disable + '" id="userid_' + data.content[i].user_id + '" data-id="' + data.content[i].user_id + '"><span>' + data.content[i].user_name + ' [' + data.content[i].mobile_phone + '] ' + '</span></li>'
					$('.nav-list-ready').append(opt);
				};
			} else {
		...
		add_link_user : function() {
			...
			.on('click', function() {
				var $this = $(this),
					tmpobj = $( '<li class="ms-elem-selection"><input type="hidden" name="user_id[]" value="' + $this.attr('data-id') + '" />' + $this.text() + '<span class="edit-list"><i class="fa fa-minus-circle ecjiafc-red del"></i></span></li>' );
				if (!$this.hasClass('disabled')) {
					tmpobj.appendTo( $( ".ms-selection .nav-list-content" ) );
					$this.addClass('disabled');
				}
