		...
		<ul>
			<li class="step-first">
				<div class="{if $refund_info.status eq '0'}step-cur{else}step-done{/if}">
					...
					<div class="m_t5">{t domain="refund"}买家申请退款{/t}<br><font class="ecjiafc-blue">{if $refund_info.add_time}{$refund_info.add_time}{/if}</font></div>	
				</div>
			</li>
			...
			<li>
				<div class="{if $refund_info.refund_status eq '1' or $refund_info.status eq '11'}step-cur{elseif $refund_info.status gt '0'}step-done{/if}">
					...
					{/if}
					<div class="m_t5">{t domain="refund"}商家处理退款申请{/t}<br><font class="ecjiafc-blue">{if $action_mer_msg.log_time}{$action_mer_msg.log_time}{/if}</font></div>
				</div>
			</li>
