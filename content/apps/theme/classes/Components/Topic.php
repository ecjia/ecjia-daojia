...
class Topic extends ComponentAbstract
{
     ...
     * @var string
     */
    protected $description = '首页专题，最多支持10个。';

    /**
     * 缩略图
     * @var string
     */
    protected $thumb = '/statics/images/thumb/module_topic.png'; //图片未添加
...
HTML;
    ...
    {
    	$mobile_topic_adsense_data = [];
    	
    	$time = \RC_Time::gmtime();
    	$positon_info = \RC_DB::table('ad_position')
    	->where('type', 'adsense')
    	->where('position_code', 'home_topic_adsense')->first();
    	
    	if (!empty($positon_info['position_id'])) {
    		$ad = \RC_DB::table('ad');
    		if (!empty($positon_info['max_number'])) {
    			$ad->take($positon_info['max_number']);
    		}
    		$list = $ad->where('start_time', '<=', $time)
    		->where('end_time', '>=', $time)
    		->where('enabled', 1)
    		->where('position_id', $positon_info['position_id'])
        ...
        return $mobile_topic_adsense_data;
    }
}