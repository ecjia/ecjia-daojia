...
<div id="navigator">
	<div class="row-fluid">
		<div class="nav_list choose_list span12">
			...
			<span class="m_r5">{t}或{/t}</span>
			<a class="data-pjax" href="{url path='theme/navigator/add_nav_list'}">{t}创建新菜单{/t}</a>
		</div>
	</div>
	...
	<div class="row-fluid nav_main">
		...
		<div class="span9 nav_edit">
			<div class="nav_edit_hd">
				<label for="nav_name">{t}菜单名称：{/t}</label><input type="hidden" name="nav_type" value="{$nav_type}" /><input id="navlist_name" type="text" name="navlist_name" value="{$nav_name}" /><input type="hidden" id="navlist_del" name="navlist_del" value="0" /><a class="btn btn-info f_r navlist_submit" href="javascript:;">{t}保存菜单{/t}</a>
			</div>
						...
						<div class="span5">
							...
							<div class="w-box" id="{$nav.type}{$id}" value="{$id}">
								...
								<div class="w-box-content hide">
									...
									<p><a class="btn btn-mini btn-danger del_nav">{t}移除{/t}</a></p>
								</div>
							</div>
							<!-- {/foreach} -->
						</div>
			...
			<div class="nav_edit_ft">
				<a class="del_navlist data-pjax" href='{url path="theme/navigator/del_navlist" args="showstate=2&del_type={$nav_type}"}'>{t}删除菜单{/t}</a>
				<a class="btn btn-info f_r navlist_submit" href="javascript:;">{t}保存菜单{/t}</a>
			</div>
		</div>
	</div>
</div>
