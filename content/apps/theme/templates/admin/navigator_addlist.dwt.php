	...
	<div class="row-fluid">
		<form action="{$form_action}" method="get" name="theForm">
			<div class="nav_list choose_list span12">
				...
				<select name="type">
					<option value="-" checked >{t domain="theme"}--选择--{/t}</option>
				...
				<span class="m_r5">{t domain="theme"}或{/t}</span><a class="data-pjax" href="{url path='theme/navigator/init'}">{t domain="theme"}返回自定义导航栏{/t}</a>
			</div>
		</form>
	</div>
			...
			<div class="accordion" id="accordion2">
				<div class="accordion-group">
					<div class="accordion-heading">
						<a class="accordion-toggle" href="#collapseTwo2" data-parent="#accordion2" data-toggle="collapse">{t domain="theme"}页面{/t}</a>
					</div>
				...
				<div class="accordion-group">
					<div class="accordion-heading">
						<a class="accordion-toggle" data-parent="#accordion2" data-toggle="collapse" href="#collapseOne2">{t domain="theme"}链接{/t}</a>
					</div>
				...
				<div class="accordion-group">
					<div class="accordion-heading">
						<a class="accordion-toggle" data-parent="#accordion2" data-toggle="collapse" href="#collapseThree2">{t domain="theme"}分类目录{/t}</a>
					</div>
					<div class="accordion-body collapse" id="collapseThree2">
						<div class="accordion-inner">
							...
							<!-- {/foreach} -->
							<p>
								<a class="checkall btn btn-mini" href="javascript:;">{t domain="theme"}全选{/t}</a><a class="btn btn-mini btn-info f_r addtolist m_b10" href="javascript:;">{t domain="theme"}添加至菜单{/t}</a>
							</p>
						</div>
					</div>
				</div>
		...
		<div class="span9 nav_edit">
			<form class="add_navlist_form" name="theForm" action="{url path='theme/navigator/update_nav_list'}" method="post">
				<div class="nav_edit_hd">
					<label for="nav_name">{t domain="theme"}菜单名称：{/t}</label><input id="nav_name" name="nav_name" type="text" /><a class="data-pjax btn btn-info f_r" href="">{t domain="theme"}创建菜单{/t}</a>
				</div>
				<div class="nav_edit_bd">
					<span class="help-block">{t domain="theme"}在上面给菜单命名，然后点击“创建菜单”。{/t}</span>
				</div>
				<div class="nav_edit_ft">
					<a class="data-pjax btn btn-info f_r" href="">{t domain="theme"}创建菜单{/t}</a>
				</div>
			</form>
		</div>
	</div>
