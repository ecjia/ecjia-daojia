...
class mh_group extends ecjia_merchant
{
    public function __construct()
    {
        ...
        ecjia_merchant_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('员工管理', 'staff'), RC_Uri::url('staff/mh_group/init')));
    }
    ...
    public function init()
    {
        ...
        $this->assign('ur_here', __('员工管理', 'staff'));
    ...
    public function add()
    {
        ...
        ecjia_merchant_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('添加员工组', 'staff')));
        $this->assign('ur_here', __('添加员工组', 'staff'));
    ...
    public function edit()
    {
        ...
        ecjia_merchant_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('编辑员工组', 'staff')));
        $this->assign('ur_here', __('编辑员工组', 'staff'));
        $this->assign('action_link', array('href' => RC_Uri::url('staff/mh_group/init'), 'text' => __('员工管理', 'staff')));
        ...
        if (empty($staff_group)) {
            $links[] = array('text' => __('返回员工组列表', 'staff'), 'href' => RC_Uri::url('staff/mh_group/init'));
            return $this->showmessage(__('该员工组不存在', 'staff'), ecjia::MSGTYPE_HTML | ecjia::MSGSTAT_ERROR, array('links' => $links));
        }
    ...
    public function remove()
    {
        ...
        if (empty($group_id)) {
        	return $this->showmessage(__('请选择要删除的员工组', 'staff'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        ...
        //提示
        return $this->showmessage(__('删除员工组成功', 'staff'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('staff/mh_group/init')));
    }
