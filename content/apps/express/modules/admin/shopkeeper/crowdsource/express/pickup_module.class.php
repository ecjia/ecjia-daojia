...
class admin_shopkeeper_crowdsource_express_pickup_module extends api_admin implements api_interface {
    public function handleRequest(\Royalcms\Component\HttpKernel\Request $request) {	
        ...
        $express_order_db->where(RC_DB::raw('eo.store_id'), $_SESSION['store_id'])->where(RC_DB::raw('eo.delivery_sn'), $delivery_sn)->where(RC_DB::raw('eo.status'), 1)->where(RC_DB::raw('eo.shipping_code'), 'ship_ecjia_express');
        $field                 = 'eo.*, oi.add_time as order_time, oi.pay_time, oi.order_amount, oi.pay_name, sf.merchants_name, sf.district as sf_district, sf.street as sf_street, sf.address as merchant_address, sf.longitude as merchant_longitude, sf.latitude as merchant_latitude';
