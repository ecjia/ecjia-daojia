					...
					<div class="accordion-inner right-scroll">
						...
						{if $express_count.online or $express_count.offline}
							...
							<div class="control-group control-group-small press-list" style="margin-bottom:0px;">
								<div class="margin-label online-list" style="margin-top:5px;margin-bottom: 5px;">{t domain="express"}在线{/t} （{if $express_count.online}{$express_count.online}{else}0{/if}）<a class="acc-in move-mod-head online-click" data-toggle="collapse" data-target="#waitgrab_online"><b class="triangle on-tri"></b></a></div>
											...
											{if $list.online_status eq '1'}
												<div class="express-user-info ex-user-div{$list.user_id}" staff_user_id="{$list.user_id}" online_status ="{$list.online_status}">
													...
													<div class="assign-div">
														 <a class="{if $express_order_count.wait_grab} re-assign {/if} "  data-msg='{t domain="express" 1={$list.name}}是否确定让  【%1】  去配送？{/t}' data-href='{url path="express/merchant/assign_express_order" args="staff_id={$list.user_id}&type={$type}"}'  >
							...
							<div class="control-group control-group-small press-list" style="margin-bottom:0px;">
								<div class="margin-label online-list" style="margin-top:5px;margin-bottom: 5px;">{t domain="express"}离线{/t} （{if $express_count.offline}{$express_count.offline}{else}0{/if}）<a class="acc-in  move-mod-head collapsed offline-click" data-toggle="collapse" data-target="#waitgrab_leave"><b class="triangle1 off-tri"></b></a></div>
