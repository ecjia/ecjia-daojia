                ...
                <div class="chat_box row" data-url='{$chat_action}' style="padding-left:15px;">
                    <div class="col-xl-8 col-lg-12 chat_content h550">
                        <div class="card">
                            <div class="card-header popover-header">
                                <h4 class="card-title">{t domain="weapp"}交谈用户：{/t}<span class="act_users">{$info.nickname}</span>
                                </h4>
                            </div>
                            <div class="card-content collapse show popover-body chat-box">
                                <div class="chat_msg clearfix">
                                    <div class="chat_msg_heading t_c">
                                        <a class="readed_message" href="javascript:;" data-chatid="{$info.uid}" data-lastid="{$message.last_id}" data-href="{$last_action}">{t domain="weapp"}查看更早的消息记录{/t}</a>
                                    </div>
                                </div>
                                <div class="card-body">
                                    <div class="chat_msg media-list">
                                        ...
                                        <div class="media {if $msg.iswechat eq 1} chat-msg-mine{else} chat-msg-you{/if} last_chat">
                                            <div class="media-body">
                                                <h5 class="media-heading">
                                                    <span class="chat_msg_date">{$msg.send_time}</span><span class="chat_user_name">{$msg.nickname}</span>
                                                </h5>
                                                <h5 class="media-text {if $msg.iswechat eq 1}text-left{/if}">
                                                    ...
                                                    <div class="img_preview">
                                                        <img class="preview_img margin_10" src="{$msg.media_content.img_url}" title='{t domain="weapp"}点击查看{/t}' data-type="image">
                                                    </div>
                                                    {/if}

                                                    {if $msg.type eq 'voice'}
                                                    <div class="img_preview">
                                                        <img class="preview_img margin_10" src="{$msg.media_content.img_url}" title='{t domain="weapp"}点击播放{/t}' data-src="{$msg.media_content.voice_url}" data-type="voice"></img>
                                                    </div>
                                                    {/if}

                                                    {if $msg.type eq 'video'}
                                                    <div class="img_preview">
                                                        <img class="preview_img margin_10" src="{$msg.media_content.img_url}" title='{t domain="weapp"}点击播放{/t}' data-src="{$msg.media_content.video_url}" data-type="video"></img>
                                                    </div>
                                                    {/if}
                                        ...
                                        <div class="media msg_clone chat-msg-mine" style="display:none">
                                            <div class="media-body">
                                                <h5 class="media-heading">
                                                    <span class="chat_msg_date"></span><span class="chat_user_name"></span>
                                                </h5>
                    ...
                    <div class="col-xl-4 col-lg-12">
                        <div class="card info_content h550">
                            <div class="card-header popover-header">
                                <h4 class="card-title">{t domain="weapp"}用户信息{/t}</h4>
                            </div>
                            <div class="card-body popover-body">
                                <div class="form-body">
                                    <div class="form-group row p_t20">
                                        <label class="col-md-5 label-control text-right">{t domain="weapp"}用户头像：{/t}</label>
                                    ...
                                    <div class="form-group row">
                                        <label class="col-md-5 label-control text-right">{t domain="weapp"}昵称：{/t}</label>
                                        <div class="col-md-7 controls p_l0">
                                    ...
                                    <div class="form-group row">
                                        <label class="col-md-5 label-control text-right">{t domain="weapp"}备注名：{/t}</label>
                                        <div class="col-md-7 controls p_l0">
											<span class="">
												{if $info.remark}
                                    ...
                                    <div class="form-group row">
                                        <label class="col-md-5 label-control text-right">{t domain="weapp"}性别：{/t}</label>
                                        <div class="col-md-7 controls p_l0">
                                            <span class="">{if $info['sex'] == 1}{t domain="weapp"}男{/t}{else if $info.sex == 2}{t domain="weapp"}女{/t}{/if}</span>
                                        </div>
                                    </div>

                                    <div class="form-group row">
                                        <label class="col-md-5 label-control text-right">{t domain="weapp"}省-市：{/t}</label>
                                        <div class="col-md-7 controls p_l0">
                                    ...
                                    <div class="form-group row">
                                        <label class="col-md-5 label-control text-right">{t domain="weapp"}用户标签：{/t}</label>
                                        <div class="col-md-7 controls p_l0">
                                            <span class="">{if $info['group_id'] eq 1}{else}{if $info['tag_name']}{$info['tag_name']}{else}{t domain="weapp"}无标签{/t}{/if}{/if}</span>
                                            <!-- {if $info.group_id neq 1 && $info.subscribe neq 0} -->
                                    ...
                                    <div class="form-group row">
                                        <label class="col-md-5 label-control text-right">{t domain="weapp"}关注时间：{/t}</label>
                                        <div class="col-md-7 controls p_l0">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-7 controls p_l0">
                                            ...
                                            <a class="ajaxremove no-underline btn btn-outline-primary m_t14" data-toggle="ajaxremove" data-msg='{t domain="weapp"}您确定要将该用户移出黑名单吗？{/t}' href='{RC_Uri::url("weapp/platform_user/black_user","openid={$info.openid}&uid={$info.uid}&type=remove_out&page={$smarty.get.page}")}' title='{t domain="weapp"}移出黑名单{/t}'>{t domain="weapp"}移出黑名单{/t}</a>
                                            <!-- {else} -->
                                            <!-- {if $info.subscribe eq 0} -->
                                            <a class="btn m_t14" disabled>{t domain="weapp"}加入黑名单{/t}</a>
                                            <!-- {else} -->
                                            <a class="ajaxremove no-underline btn btn-outline-primary m_t14" data-toggle="ajaxremove" data-msg='{t domain="weapp"}您确定要将该用户加入黑名单吗？{/t}' href='{RC_Uri::url("weapp/platform_user/black_user","openid={$info.openid}&uid={$info.uid}&page={$smarty.get.page}")}' title='{t domain="weapp"}加入黑名单{/t}'>{t domain="weapp"}加入黑名单{/t}</a>
                                            <!-- {/if} -->
                                            <!-- {/if} -->
                                        </div>
                                    </div>
                    ...
                    <div class="col-xl-12 col-lg-12 material-table m_t20 p_l0">
                        <ul class="nav nav-tabs nav-only-icon nav-top-border no-hover-bg">
                            <li class="nav-item" data-type="text">
                                <a class="nav-link active" data-toggle="tab" title='{t domain="weapp"}文本{/t}'><i class="fa fa-pencil"> {t domain="weapp"}文字{/t}</i></a>
                            </li>
                            <li class="nav-item" data-type="image">
                                <a class="nav-link" data-toggle="tab" title='{t domain="weapp"}图片{/t}'><i class="fa fa-file-image-o"> {t domain="weapp"}图片{/t}</i></a>
                            </li>
                        </ul>
                        ...
                        <span class="tip_info">{t domain="weapp"}（只有48小时内给公众号发送过信息的粉丝才能接收到信息）{/t}</span>
                        <span class="word_info">{t domain="weapp"}（最多输入600字）{/t}</span>
                        ...
                        <a class="btn f_r btn-info send_msg" href="javascript:;">{t domain="weapp"}发送{/t}</a>
                    </div>
                </div>
            </div>
        </div>
...
<!-- {include file="./library/weapp_show_message.lbi.php"} -->
