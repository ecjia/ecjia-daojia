                    ...
                    <div class="card-body">
                        <div class="form-body">
                            <div class="form-group row">
                                <label class="col-lg-2 label-control text-right">{t domain="weapp"}客服账号：{/t}</label>
                                <div class="col-lg-8 controls">
                                    ...
                                    <span class="help-block">{t domain="weapp"}账号格式：账号前缀@公众号微信号{/t}</span>
                                    <!-- {/if} -->
                                </div>
                            ...
                            <div class="form-group row">
                                <label class="col-lg-2 label-control text-right">{t domain="weapp"}客服昵称：{/t}</label>
                                <div class="col-lg-8 controls">
                            ...
                            <!-- {if $list.id} -->
                            <div class="form-group row">
                                <label class="col-lg-2 label-control text-right">{t domain="weapp"}客服头像：{/t}</label>
                                <div class="col-lg-8 controls">
                                    <div class="fileupload {if $list.kf_headimgurl}fileupload-exists{else}fileupload-new{/if}" data-provides="fileupload">
                                        <div class="fileupload-preview fileupload-exists thumbnail" style="width: 50px; height: 50px; line-height: 50px;">
                                            ...
                                            <img src="{$list.kf_headimgurl}" alt='{t domain="weapp"}图片预览{/t}'/>
                                            {/if}
                                        </div>
                                        ...
                                        <a class="btn btn-danger {if !$list.kf_headimgurl}fileupload-exists{else}fileupload-new{/if}" {if !$list.kf_headimgurl}data-dismiss="fileupload" href="javascript:;" {/if}>{t domain="weapp"}删除{/t}</a>
                                    </div>
                                </div>
                            </div>
                            <!-- {/if} -->
                            ...
                            <div class="form-group row">
                                <label class="col-lg-2 label-control text-right">{t domain="weapp"}状态：{/t}</label>
                                <div class="col-lg-8 controls">
                                    <input type="radio" id="status_1" name="status" value="1" {if $list.status eq 1}checked{/if}><label for="status_1">{t domain="weapp"}开启{/t}</label>
                                    <input type="radio" id="status_0" name="status" value="0" {if $list.status eq 0}checked{/if}><label for="status_0">{t domain="weapp"}关闭{/t}</label>
                                    <div class="help-block">{t domain="weapp"}如果状态为关闭，则微信端不添加该客服{/t}</div>
                                </div>
                            </div>

                            <!-- {if $list.id} -->
                            <div class="form-group row">
                                <label class="col-lg-2 label-control text-right">{t domain="weapp"}绑定微信号：{/t}</label>
                                <div class="col-lg-8 controls">
                                    {if $list.status eq 1}
                                        {if $list.kf_wx}
                                            {$list.kf_wx}
                                        {elseif $list.invite_wx}
                                            {if $list.invite_status eq 'waiting'}
                                                ...
                                                {t domain="weapp"}邀请绑定待确认{/t}<a class="hint--bottom hint--rounded" data-hint='{t domain="weapp" 1={$list.invite_wx}}绑定邀请已发送至 %1 的微信，请去微信客户端确认后即可绑定{/t}'><i class="fontello-icon-help-circled"></i></a>
                                                </span>
                                            {elseif $list.invite_status eq 'rejected'}
                                                <span class="ecjiafc-999">
                                                {t domain="weapp"}邀请绑定被拒绝{/t}<a class="hint--bottom hint--rounded" data-hint='{t domain="weapp"}由于对方已拒绝绑定，可重新进行绑定。{/t}'><i class="fontello-icon-help-circled"></i></a>
                                                </span><br/>
                                                <a class="bind_wx" data-toggle="modal" href="#bind_wx" title='{t domain="weapp"}绑定微信号{/t}' data-val="{$list.kf_account}">{t domain="weapp"}重新绑定{/t}</a>
                                            {elseif $list.invite_status eq 'expired'}
                                                <span class="ecjiafc-999">
                                                {t domain="weapp"}邀请绑定过期{/t}<a class="hint--bottom hint--rounded" data-hint='{t domain="weapp"}由于邀请绑定已过期，可重新进行绑定。{/t}'><i class="fontello-icon-help-circled"></i></a>
                                                </span><br/>
                                                <a class="bind_wx" data-toggle="modal" href="#bind_wx" title='{t domain="weapp"}绑定微信号{/t}' data-val="{$list.kf_account}">{t domain="weapp"}重新绑定{/t}</a>
                                            {/if}
                                        {else}
                                            <a class="bind_wx" data-toggle="modal" href="#bind_wx" title='{t domain="weapp"}绑定微信号{/t}' data-val="{$list.kf_account}">{t domain="weapp"}绑定微信号{/t}</a>
                                        {/if}
                                    {else}
                                        <span class="ecjiafc-999">{t domain="weapp"}该客服账号已停用{/t}</span><br>
                                        <a class="bind_wx" data-toggle="modal" href="#bind_wx" title='{t domain="weapp"}绑定微信号{/t}' data-val="{$list.kf_account}">{t domain="weapp"}重新绑定{/t}</a>
                                    {/if}
                                </div>
                            </div>
...
<div class="modal fade text-left" id="bind_wx">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form class="form" method="post" name="bind_form" action="{url path='weapp/platform_customer/bind_wx'}&id={$list.id}">
                <div class="card-body">
                    <div class="form-body">
                        <div class="form-group row">
