<!-- {if !$list} -->
<div class="empty_material">{t domain="weapp"}暂无素材{/t}</div>
<!-- {else} -->
<!-- {if $type eq 'news'} -->
    ...
    <div class="wmk_grid ecj-wookmark wookmark_list material_pictures">
        <ul class="wookmark-goods-photo move-mod nomove wookmark-news">
            <div class="weui-desktop-media__list-col">
                ...
                {if ($k+4) % 4 == 0}
                <!-- {include file="library/weapp_choose_material_newchild_list.lbi.php"} -->
                {/if}
                <!-- {/foreach} -->
            </div>
...
<li class="img_item">
    <label class="img_item_bd">
        <div class="pic_box">
            <img class="pic" src="{if $val.type eq 'voice' || $val.type eq 'video'}{$val.thumb}{else}{$val.file}{/if}" data-media="{$val.media_id}" data-id="{$val.id}"/>
        </div>
