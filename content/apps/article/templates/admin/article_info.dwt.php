	...
	<div class="span12">
		<div class="tabbable">
			...
			<ul class="nav nav-tabs">
				<li class="active"><a href="#tab1" data-toggle="tab">{t domain="article"}通用信息{/t}</a></li>
				<!--<li><a href="#tab2" data-toggle="tab">{t domain="article"}文章内容{/t}</a></li> -->
				<li><a class="data-pjax" href='{url path="article/admin/link_goods" args="id={$smarty.get.id}{if $publishby}&publishby={$publishby}{/if}"}'>{t domain="article"}关联商品{/t}</a></li>
			</ul>
			{/if}
			<form class="form-horizontal" action="{$form_action}" method="post" enctype="multipart/form-data" name="infoForm" data-edit-url="{RC_Uri::url('article/admin/edit')}">
					...
					<fieldset>
							<div class="row-fluid edit-page editpage-rightbar">
								<div class="left-bar move-mod">
									...
									<div class="control-group" >
									 	<label>{t domain="article"}外部链接：{/t}</label>
										<div>
											<input type="text" name="link_url" class="span10" value="{if $article.link neq ''}{$article.link|escape}{/if}" />
											<span class="help-block">{t domain="article"}若输入外部链接，则该链接优先{/t}</span>
										</div>
									</div>
										...
										<div class="accordion-group">
											<div class="accordion-heading">
												<a class="accordion-toggle collapsed acc-in move-mod-head" data-toggle="collapse" data-target="#goods_info_term_meta">
													<strong>{t domain="article"}自定义栏目{/t}</strong>
												</a>
											</div>
											<div class="accordion-body in" id="goods_info_term_meta">
												<div class="accordion-inner">
													...
													<table class="table smpl_tbl ">
														<thead>
															<tr>
																<td class="span4">{t domain="article"}名称{/t}</td>
																<td>{t domain="article"}值{/t}</td>
															</tr>
														</thead>
													...
													<!-- {/if} -->
 													...
 													<!-- 编辑区域 -->
 													<label><b>{t domain="article"}添加自定义栏目：{/t}</b></label>
															...
															<tbody class="term_meta_edit" data-id="{$article.article_id}" data-active="{url path='article/admin/update_term_meta'}">
																<tr>
																	<td>
																		...
																		<div><a data-toggle="add_new_term_meta" href="javascript:;">{t domain="article"}添加新栏目{/t}</a></div>
 																		<!-- {else} -->
																		<input class="span12" type="text" name="term_meta_key" value="" />
																		<!-- {/if} -->
																		<a class="btn m_t5" data-toggle="add_term_meta" href="javascript:;">{t domain="article"}添加自定义栏目{/t}</a>
																	</td>
										...
										<div class="accordion-group">
											<div class="accordion-heading">
												<a class="accordion-toggle collapsed move-mod-head" data-toggle="collapse" data-target="#goods_info_area_author">
													<strong>{t domain="article"}作者信息{/t}</strong>
												</a>
											</div>
											<div class="accordion-body in in_visable collapse" id="goods_info_area_author">
												<div class="accordion-inner">
													<div class="control-group control-group-small" >
														<label class="control-label">{t domain="article"}作者名称：{/t}</label>
														<div class="span8">
															<input type="text" name="author"  value="{$article.author|escape}"/>
														</div>
													</div>
													<div class="control-group control-group-small" >
														<label class="control-label">{t domain="article"}作者邮箱：{/t}</label>
														<div class="span8">
										...
										<div class="accordion-group">
											<div class="accordion-heading">
												<a class="accordion-toggle collapsed move-mod-head" data-toggle="collapse" data-target="#goods_info_area_cat">
													<strong>{t domain="article"}分类信息{/t}</strong>
												</a>
											</div>
											<div class="accordion-body in in_visable collapse" id="goods_info_area_cat">
												<div class="accordion-inner">
													<!-- {if $article.cat_id >= 0} -->
													<div class="control-group control-group-small" >
														<label class="control-label">{t domain="article"}文章分类：{/t}</label>
													...
													<!-- {if $article.cat_id >= 0} -->
													<div class="control-group control-group-small" >
														<label class="control-label">{t domain="article"}文章类型：{/t}</label>
														<div class="control span8">
															<select name="article_type">
																<option value="article">{t domain="article"}请选择...{/t}</option>
													...
													<div class="control-group control-group-small" >
														<label class="control-label">{t domain="article"}推荐类型：{/t}</label>
														<div class="span8 chk_radio">
															<input type="radio" class="uni_style" name="suggest_type" value="stickie" {if $article.suggest_type eq 'stickie'}checked{/if}><span>{t domain="article"}置顶{/t}</span>
															<input type="radio" class="uni_style" name="suggest_type" value="0" {if $article.suggest_type eq '0'}checked{/if}><span>{t domain="article"}默认{/t}</span>
														</div>
													</div>
													<div class="control-group control-group-small" >
														<label class="control-label">{t domain="article"}状态审核：{/t}</label>
														<div class="span8 chk_radio">
															<input type="radio" class="uni_style" name="article_approved" value="1" {if $article.article_approved eq 1}checked{/if}><span>{t domain="article"}通过{/t}</span>
															<input type="radio" class="uni_style" name="article_approved" value="0" {if $article.article_approved eq 0}checked{/if}><span>{t domain="article"}待审核{/t}</span>
															<input type="radio" class="uni_style" name="article_approved" value="trash" {if $article.article_approved eq 'trash'}checked{/if}><span>{t domain="article"}回收站{/t}</span>
															<input type="radio" class="uni_style" name="article_approved" value="spam" {if $article.article_approved eq 'spam'}checked{/if}><span>{t domain="article"}垃圾文章{/t}</span>
														</div>
													</div>
													<!-- {else} -->
													...
													<button class="btn btn-gebo" type="submit">{t domain="article"}发布{/t}</button>
													{else}
										...
										<div class="accordion-group">
											<div class="accordion-heading">
												<a class="accordion-toggle collapsed move-mod-head" data-toggle="collapse" data-target="#goods_info_area_upfile">
													<strong>{t domain="article"}上传文件{/t}</strong>
												</a>
											</div>
													...
													<div>
														{if $article.file_url neq ''}
															{if $article.is_file}
														    	...
														    	<div class="h100 ecjiaf-wwb">{t domain="article"}文件地址：{/t}{$article.file_url}</div>
													       	{else}
													       		<div class="t_c">
														...
														<div>
															<div data-provides="fileupload" class="fileupload fileupload-new m_t10"><input type="hidden" value="" name="">
																<span class="btn btn-file"><span class="fileupload-new">{t domain="article"}上传文件{/t}</span><span class="fileupload-exists">{t domain="article"}修改文件{/t}</span><input type="file" name="file"></span>
										...
										<div class="accordion-group">
											<div class="accordion-heading">
												<a class="accordion-toggle collapsed move-mod-head" data-toggle="collapse" data-target="#article_cover_upfile">
													<strong>{t domain="article"}文章封面{/t}</strong>
												</a>
											</div>
													...
													<div class="accordion-inner" style="padding:16px 15px;">
														<div class="fileupload fileupload-new m_b0" data-provides="fileupload">
															...
															<span class="btn btn-file">
																<span  class="fileupload-new">{t domain="article"}浏览{/t}</span>
																<span  class="fileupload-exists">{t domain="article"}修改{/t}</span>
																<input type='file' name='cover_image' size="35"/>
															</span>
															<a class="btn fileupload-exists" data-dismiss="fileupload" href="#">{t domain="article"}删除{/t}</a>
														</div>
													</div>
												{else}
				...
				<h3 class="heading">{t domain="article"}文章内容{/t}</h3>
				<div class="row-fluid">
					<div class="span12">
