...
<div class="row-fluid batch" >
	<form method="post" action="{$search_action}{if $type}&type={$type}{/if}{if $publishby}&publishby={$publishby}{/if}" name="searchForm">
		...
		<div class="choose_list f_r" >
			<input type="text" name="keywords" value="{$smarty.get.keywords}" placeholder='{t domain="article"}输入用户名,文章标题或评论关键字{/t}'/>
				...
				<tr>
				    ...
				    <td>
						...
						<!-- {elseif $list.comment_approved eq '0'} -->
								<span style="color:#08c;">{t domain="article"}待审核{/t}</span>
						<!-- {elseif $list.comment_approved eq 'trash'} -->
								<span class="ecjiafc-red">{t domain="article"}回收站{/t}</span>
						<!-- {elseif $list.comment_approved eq 'spam'} -->
								<span class="ecjiafc-red">{t domain="article"}垃圾评论{/t}</span>
						<!-- {/if} -->
					</td>
				</tr>
				<!-- {foreachelse} -->
