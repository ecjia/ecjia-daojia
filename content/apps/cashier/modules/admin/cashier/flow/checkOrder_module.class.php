...
// 		);
	...
	private function _processAddgoods($addgoods = array(), $store_id = 0, $pendorder_id = 0, $flow_type = null)
	{
		...
		if (!empty($products_goods)) {
			...
			$goods = RC_DB::table('goods')->where('store_id', $store_id)->where('is_on_sale', 1)->where('is_delete', 0)
			->where(function($query) use ($addgoods){
				$query->where('goods_sn', trim($addgoods['goods_sn']))
				->orWhere('goods_barcode', $addgoods['goods_sn']);
			})
			->first();
		}
		if (empty($goods)) {
