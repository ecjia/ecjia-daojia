...
class checkin_integral_module extends api_front implements api_interface {
	...
	public function handleRequest(\Royalcms\Component\HttpKernel\Request $request) {	
		...
		$checkin_info = RC_DB::table('mobile_checkin')->where('user_id', $_SESSION['user_id'])->where('checkin_time', '>=', $start_time)->where('checkin_time', '<=', $end_time)->first();
		if (empty($checkin_info)) {
			...
			RC_DB::table('mobile_checkin')->insert(array(
			...
			if (ecjia::config('checkin_award_open')) {
				...
				if ($checkin_extra_award['day'] > 0) {
					...
					//					->select();
					$checkin_list				= RC_DB::table('mobile_checkin')->where('user_id', $_SESSION['user_id'])->where('checkin_time', '>=', $continuous_start_time)->where('checkin_time', '<=', $end_time)->where('integral', '!=', $continuous_signin_integral)->get();
					if (count($checkin_list) == $checkin_extra_award['day']) {
						...
						RC_DB::table('mobile_checkin')->where('user_id', $_SESSION['user_id'])->where('checkin_time', $time)->where('source', 'ECJia_app')->update(array('integral' => $integral));
					}
				}
