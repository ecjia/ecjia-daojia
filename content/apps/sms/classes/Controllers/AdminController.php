...
class AdminController extends AdminBase
{
    ...
    public function init()
    {
        ...
        ecjia_screen::get_current_screen()->set_help_sidebar(
            ...
            '<p>' . __('<a href="https://ecjia.com/wiki/帮助:ECJia智能后台:短信记录" target="_blank">' . __('关于短信记录列表帮助文档', 'sms') . '</a>', 'sms') . '</p>'
        );
     ...
     */
   	private function get_sendlist() {
    	...
    	$db_sms_sendlist = RC_DB::connection(config('ecjia.database_connection', 'default'))->table('sms_sendlist');
    	...
    	if (!empty($row)) {
    		foreach ($row AS $key => $val) {
    			...
    			$row[$key]['channel_name'] = RC_DB::connection(config('ecjia.database_connection', 'default'))->table('notification_channels')->where('channel_code', $val['channel_code'])->value('channel_name');
    		}
    	}
