...
class platform extends ecjia_platform
{
    public function __construct()
    {
        ...
        RC_Script::enqueue_script('platform_activity', RC_App::apps_url('statics/platform-js/platform_activity.js', __FILE__), array(), false, 1);
        RC_Script::localize_script('platform_activity', 'js_lang', config('app-market::jslang.market_page'));
    ...
    public function close_activity()
    {
        ...
        $code = trim($_GET['code']);
        if (!empty($code)) {
            $activity_name = RC_DB::table('market_activity')->where('activity_group', $code)->where('store_id', $_SESSION['store_id'])->value('activity_name');
    ...
    public function activity_prize()
    {
        ...
        $id = RC_DB::table('market_activity')->where('activity_group', $activity_code)->where('store_id', $_SESSION['store_id'])->where('wechat_id', $wechat_id)->value('activity_id');
    ...
    private function get_activity_record_list($activity_id = 0)
    {
        ...
        if (!empty($res)) {
            foreach ($res as $key => $val) {
                ...
                $res[$key]['prize_type'] = RC_DB::table('market_activity_prize')->where('prize_id', $val['prize_type'])->value('prize_type');
            }
        }
