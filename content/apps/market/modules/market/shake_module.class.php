...
class market_shake_module extends api_front implements api_interface {
    public function handleRequest(\Royalcms\Component\HttpKernel\Request $request) {
		...
		if ((!is_array($location) || empty($location['longitude']) || empty($location['latitude']))) {
			return new ecjia_error('invalid_parameter', sprintf(__('请求接口%s参数无效', 'market'), __CLASS__));
		}
	...
	*/
	...
	private function _ProcessTypeStore ($prize_info = array(), $user_id = 0, $location = array(), $city_id = 0)
	{
		...
		if (!empty($options['store_id'])) {
			...
			if (!empty($store_info)) {
				$shop_logo = RC_DB::table('merchants_config')->where('store_id', $store_info['store_id'])->where('code', 'shop_logo')->value('value');
				$shop_notice = RC_DB::table('merchants_config')->where('store_id', $store_info['store_id'])->where('code', 'shop_notice')->value('value');
