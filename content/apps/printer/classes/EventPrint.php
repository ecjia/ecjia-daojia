...
class EventPrint extends RC_Object
{
    ...
    public function send($machine, array $template_var)
    {
        if (!$this->model || !$this->event) {
            return new ecjia_error('not_found_object', sprintf(__('请先使用%s或%s方法设置参数', 'printer'), 'setTemplateModel', 'setEvent'));
        }
    ...
    public function resend($id)
    {
        ...
        if (empty($model)) {
            return new ecjia_error('not_found_print_id', __('没有找到此打印记录', 'printer'));
        }

        $tips = __('提示：此单为重新打印订单', 'printer');
