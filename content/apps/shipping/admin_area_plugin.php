...
class admin_area_plugin extends ecjia_admin
{
    ...
    public function init()
    {
        ...
        $ship_areas_list   = Ecjia\App\Shipping\ShippingArea::GetShipareasList($args);
        ...
        $this->assign('edit_url', RC_Uri::url('shipping/admin_area_plugin/edit'));
    ...
    public function update()
    {
        ...
        if ($ship_area_count > 0) {
            ...
            RC_DB::table('shipping_area')->where('shipping_area_id', $shipping_area_id)->update($data);
            ...
            $data_region = RC_DB::table('regions')->select('region_id', 'parent_id')->get();
    ...
    public function remove_area()
    {
        ...
        if ($id > 0) {
            $row           = RC_DB::table('shipping_area')->where('shipping_area_id', $id)->first();
    ...
    public function multi_remove()
    {
        ...
        $code        = !empty($_GET['code']) ? trim($_GET['code']) : '';
        $ids         = explode(',', $ids);
        $row         = RC_DB::table('shipping_area')->whereIn('shipping_area_id', $ids)->get();
