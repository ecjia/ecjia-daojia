...
class shipping_factory {
	...
	public function set_adapter($adapter_name, $adapter_config = array()) {
		if (!is_string($adapter_name)) 
			...
			$shipping_plugins = ecjia_config::instance()->get_addon_config('shipping_plugins', true);
		    if (isset($shipping_plugins[$code])) {
		        RC_Plugin::load_files($shipping_plugins[$code]);
		    }
			$this->adapter_instance =  RC_Hook::apply_filters('shipping_factory_adapter_instance', null, $adapter_config); 
			if (!($this->adapter_instance instanceof shipping_abstract))
				return false;
		}
	...
	public function __call($method_name, $method_args) {
		...
		elseif (
			...
			&& ($this->adapter_instance instanceof shipping_abstract)
