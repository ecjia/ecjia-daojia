...
class wechat_api_hooks
{

}

if (defined('RC_SITE') && RC_SITE == 'platform')
{
    ...
    Ecjia\App\Platform\Plugin\PlatformAbstract::macro('hasBindUser', function ()
    {
        ...
        if (empty($ect_uid)) {
            //获取微信绑定的connect_user对象
            $connect_user = $wechat_user->getConnectUser();
            //获取connect_user表中绑定的ecjia会员的userid
        ...
        //未绑定用户, 微信用户表已有绑定，自动关联
        else {
            //获取微信绑定的connect_user对象
            $connect_user = $wechat_user->getConnectUser();
            //合并ect_uid旧的数据处理
            if ($connect_user->checkOpenId()) {
                $connect_user->bindUser($ect_uid);
            }
            else {
                $connect_user->saveOpenId('', '', '', 7200);
                $connect_user->bindUser($ect_uid);
            }
