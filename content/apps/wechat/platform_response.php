...
class platform_response extends ecjia_platform
{
    ...
    public function reply_subscribe()
    {
        ...
        ecjia_platform_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('关注自动回复', 'wechat')));
        $this->assign('ur_here', __('自动回复', 'wechat'));
                ...
                if (!empty($data)) {
                    foreach ($data as $k => $v) {
                        if (!empty($v['file'])) {
                            ...
                            $subscribe['child'][$k]['add_time'] = RC_Time::local_date(__('Y年m月d日', 'wechat'), $v['add_time']);
                        } else {
    ...
    public function reply_subscribe_insert()
    {
        ...
        if (empty($id)) {
            ...
            $file     = RC_DB::table('wechat_media')->where('wechat_id', $wechat_id)->where('id', $media_id)->value('file');

            if ($reply_type == 'text') {
                $this->admin_log(sprintf(__('%s，回复类型是 文字', 'wechat'), $content), 'add', 'reply_subscribe');
            } elseif ($reply_type == 'image') {
                $this->admin_log(sprintf(__('%s，回复类型是 图片', 'wechat'), $file), 'add', 'reply_subscribe');
            } elseif ($reply_type == 'voice') {
                $this->admin_log(sprintf(__('%s，回复类型是 语音', 'wechat'), $file), 'add', 'reply_subscribe');
            } elseif ($reply_type == 'video') {
                $this->admin_log(sprintf(__('%s，回复类型是 视频', 'wechat'), $file), 'add', 'reply_subscribe');
            }
            if ($id) {
                return $this->showmessage(__('添加成功', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('wechat/platform_response/reply_subscribe')));
            } else {
            ...
            if ($reply_type == 'text') {
                $this->admin_log(sprintf(__('%s，回复类型是 文字', 'wechat'), $content), 'edit', 'reply_subscribe');
            } elseif ($reply_type == 'image') {
                $this->admin_log(sprintf(__('%s，回复类型是 图片', 'wechat'), $file), 'edit', 'reply_subscribe');
            } elseif ($reply_type == 'voice') {
                $this->admin_log(sprintf(__('%s，回复类型是 语音', 'wechat'), $file), 'edit', 'reply_subscribe');
            } elseif ($reply_type == 'video') {
                $this->admin_log(sprintf(__('%s，回复类型是 视频', 'wechat'), $file), 'edit', 'reply_subscribe');
            }

            if ($id) {
                return $this->showmessage(__('编辑成功', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('wechat/platform_response/reply_subscribe')));
            } else {
                return $this->showmessage(__('添加失败', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
        }
    ...
    public function reply_msg()
    {
        ...
        ecjia_platform_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('消息自动回复', 'wechat')));
        $this->assign('ur_here', __('自动回复', 'wechat'));
    ...
    public function reply_msg_insert()
    {
        ...
        if (empty($id)) {
            ...
            if (!empty($content)) {
                $this->admin_log(sprintf(__('%s，回复类型是 文字', 'wechat'), $content), 'add', 'reply_msg');
            } else {
                if ($info['type'] == 'image') {
                    $this->admin_log(sprintf(__('%s，回复类型是 图片', 'wechat'), $info['file']), 'add', 'reply_msg');
                } elseif ($info['type'] == 'voice') {
                    $this->admin_log(sprintf(__('%s，回复类型是 语音', 'wechat'), $info['file']), 'add', 'reply_msg');
                } elseif ($info['type'] == 'video') {
                    $this->admin_log(sprintf(__('%s，回复类型是 视频', 'wechat'), $info['file']), 'add', 'reply_msg');
                }
            }
            if ($id) {
                return $this->showmessage(__('添加成功', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('wechat/platform_response/reply_msg')));
            } else {
                return $this->showmessage(__('添加失败', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
        } else {
            ...
            if (!empty($content)) {
                $this->admin_log(sprintf(__('%s，回复类型是 文字', 'wechat'), $content), 'edit', 'reply_msg');
            } else {
                if ($info['type'] == 'image') {
                    $this->admin_log(sprintf(__('%s，回复类型是 图片', 'wechat'), $info['file']), 'edit', 'reply_msg');
                } elseif ($info['type'] == 'voice') {
                    $this->admin_log(sprintf(__('%s，回复类型是 语音', 'wechat'), $info['file']), 'edit', 'reply_msg');
                } elseif ($info['type'] == 'video') {
                    $this->admin_log(sprintf(__('%s，回复类型是 视频', 'wechat'), $info['file']), 'edit', 'reply_msg');
                }
            }
            if ($id) {
                return $this->showmessage(__('编辑成功', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('wechat/platform_response/reply_msg')));
            } else {
                return $this->showmessage(__('编辑失败', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
        }
    }
    ...
    {
        ecjia_platform_screen::get_current_screen()->set_subject(__('关键词回复', 'wechat'));

        $this->admin_priv('wechat_response_manage');

        $this->assign('ur_here', __('关键词回复', 'wechat'));
        ecjia_platform_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('关键词回复', 'wechat')));

        $wechat_id = $this->platformAccount->getAccountID();
        $this->assign('form_action', RC_Uri::url('wechat/platform_response/reply_keywords'));
        $this->assign('action_link', array('text' => __('添加规则', 'wechat'), 'href' => RC_Uri::url('wechat/platform_response/reply_keywords_add')));
    ...
    public function reply_keywords_add()
    {
        ...
        ecjia_platform_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('关键词回复', 'wechat'), RC_Uri::url('wechat/platform_response/reply_keywords')));
        $this->assign('action_link', array('href' => RC_Uri::url('wechat/platform_response/reply_keywords'), 'text' => __('关键词回复', 'wechat')));
        ...
        if (!empty($id)) {
            $this->assign('ur_here', __('编辑规则', 'wechat'));
            ecjia_platform_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('编辑规则', 'wechat')));

            $this->assign('id', $id);
            $data = $this->get_rule_info($id);
            $this->assign('data', $data);
        } else {
            $this->assign('ur_here', __('添加规则', 'wechat'));
            ecjia_platform_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('添加规则', 'wechat')));
    ...
    public function reply_keywords_insert()
    {
        ...
        if (is_ecjia_error($wechat_id)) {
            return $this->showmessage(__('添加失败，请先添加公众号，再进行后续操作', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        ...
        if ($data['rule_name'] == '') {
            return $this->showmessage(__('请填写规则名称', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }

        if ($rule_keywords == '') {
            return $this->showmessage(__('请至少填写1个关键词', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }

        if ($data['content'] == '' && empty($data['media_id'])) {
            return $this->showmessage(__('请填写或选择回复内容！', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        $data['type'] = 'keywords';
        ...
        if (!empty($id)) {
            ...
            if ($is_only != 0) {
                return $this->showmessage(sprintf(__('规则名称 %s 已存在', 'wechat'), $data['rule_name']), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
        } else {
        ...
        if (!empty($rule_keywords_list)) {
            foreach ($rule_keywords_list as $v) {
                if (in_array($v['rule_keywords'], $rule_keywords, true)) {
                    return $this->showmessage(sprintf(__('关键词 %s 已存在', 'wechat'), $v['rule_keywords']), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
                }
            }
        }
        ...
        foreach ($count as $k => $c) {
            if ($c > 1) {
                return $this->showmessage(sprintf(__('关键词 %s 重复', 'wechat'), $k), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
            }
        }
        if (!empty($id)) {
            $this->admin_priv('wechat_response_update', ecjia::MSGTYPE_JSON);
        ...
        if ($update) {
            return $this->showmessage(__('编辑成功', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('wechat/platform_response/reply_keywords_add', array('id' => $id))));
        } else {
            $links[] = array('text' => __('返回关键词回复', 'wechat'), 'href' => RC_Uri::url('wechat/platform_response/reply_keywords'));
            $links[] = array('text' => __('继续添加规则', 'wechat'), 'href' => RC_Uri::url('wechat/platform_response/reply_keywords_add'));
            return $this->showmessage(__('添加成功', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('links' => $links, 'pjaxurl' => RC_Uri::url('wechat/platform_response/reply_keywords_add', array('id' => $id))));
        }
    }
    ...
    public function remove_rule()
    {
        ...
        if (is_ecjia_error($wechat_id)) {
            return $this->showmessage(__('删除失败，请先添加公众号，再进行后续操作', 'wechat'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
        }
        ...
        if (!empty($list)) {
            foreach ($list as $key => $val) {
                ...
                if (!empty($val['media_id'])) {
                    ...
                    if (!empty($media)) {
                        $media['add_time'] = RC_Time::local_date(__('n月j日', 'wechat'), $media['add_time']);
                    ...
                    if ($val['reply_type'] == 'news' && $is_articles != 0) {
                        ...
                        foreach ($info as $k => $v) {
                            if (!empty($v['file'])) {
                                ...
                                $list[$key]['medias'][$k]['add_time'] = RC_Time::local_date(__('Y年m月d日', 'wechat'), $v['add_time']);
                            } else {
                ...
                if (!empty($data)) {
                    foreach ($data as $k => $v) {
                        if (!empty($v['file'])) {
                            ...
                            $list['child'][$k]['add_time'] = RC_Time::local_date(__('Y年m月d日', 'wechat'), $v['add_time']);
                        } else {
    ...
    private function get_article_list($id, $type)
    {
        ...
        if (!empty($data)) {
            foreach ($data as $k => $v) {
                ...
                $article['file'][$k]['add_time'] = RC_Time::local_date(__('Y年m月d日', 'wechat'), $v['add_time']);
