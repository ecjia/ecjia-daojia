...
;(function(ecjia, $) {
	ecjia.platform = {
		...
		init : function() {
            ...
            /* 设置ajax提交默认项 */ 
            ecjia.platform.set_ajax_submit_option();

            /* 设置提示插件 */
            ecjia.platform.set_sticky();
...
//			ecjia.platform.search();
         ...
         */
        set_sticky : function() {
            if (!ecjia.platform.sticky && $.fn.sticky) 
            	ecjia.platform.sticky = function(note, options, callback) { 
		...
		showmessage : function(options) {
			...
			var alert_obj = $('<div class="col-12 mt-1 mb-0 staticalert alert alert-dismissable ' + options.state + ' ui_showmessage">' + _close + options.message + '</div>');
