...
class platform_factory  {
	...
	public function set_adapter($adapter_name, $adapter_config = array()) {
		if (!is_string($adapter_name)) 
			...
			$integrate_plugins = ecjia_config::instance()->get_addon_config('platform_plugins', true);
		    if (isset($integrate_plugins[$code])) {
		        RC_Plugin::load_files($integrate_plugins[$code]);
		    }
			$this->adapter_instance =  RC_Hook::apply_filters('platform_factory_adapter_instance', null, $adapter_config);
			if (!($this->adapter_instance instanceof platform_abstract))
			    return false;
		}
	...
	public function has_adapter() {
	    if (is_object($this->adapter_instance) && $this->adapter_instance instanceof platform_abstract) {
	        return true;
	    }
	...
	public function __call($method_name, $method_args) {
		if (method_exists($this, $method_name)) {
			...
			&& ($this->adapter_instance instanceof platform_abstract)
