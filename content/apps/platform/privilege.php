                ...
                if (intval($time_gap) < 30) {
                    ...
                    if ($user_type == 'admin') {
                        ...
                        if ($user->getActionList()) {
                            ...
                            $this->assign('error_message', __('抱歉！该用户没有分配公众平台登录权限。', 'platform'));
                        }
                    ...
                    $this->assign('error_message', __('抱歉！请求超时。', 'platform'));
                }

            } //参数不全，错误提示
            else {
                $this->assign('error_message', __('传参出错。', 'platform'));
            }
        } //参数authcode接收失败，错误提示
        else {
            $this->assign('error_message', __('抱歉！数据丢失，登录失败。', 'platform'));
        }
