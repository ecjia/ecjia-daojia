			...
			<li class="step-last">
				<div class="{if $status eq 2 || $status eq 3}step-cur{/if}">
					...
					<div class="m_t5">
						...
						{else if $status eq 2}
                        {t domain="commission"}已审核，充值完成{/t}
						{else if $status eq 3}
			...
			<div class="accordion-body in collapse" id="telescopic1">
				<table class="table table-oddtd m_b0">
					<tbody class="first-td-no-leftbd">
						...
						<tr>
							<td><div align="right"><strong>{t domain="commission"}充值金额：{/t}</strong></div></td>
							<td class="amount_price">{$data.format_amount}</td>
							<td><div align="right"><strong>{t domain="commission"}充值方式：{/t}</strong></div></td>
							<td>{$data.pay_name}</td>
						</tr>
						<tr>
                            <td><div align="right"><strong>{t domain="commission"}充值账号：{/t}</strong></div></td>
                            <td>{$data.merchants_name}</td>
