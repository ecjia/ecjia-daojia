...
//         RC_Logger::getLogger('info')->info($data);
        ...
        $db_store_user =  RC_DB::table('store_users as s')
            ->where(RC_DB::raw('s.store_id'), '!=' , $order_info['store_id'])
...
//                    if($data['order_type'] == 'refund') {
//                        with(new AgencysaleStoreBill($affiliate, $order_info))->refund();
//                    } else {
//                        with(new AgencysaleStoreBill($affiliate, $order_info))->bill();
//                    }
//                    //vip
//                    with(new AgencysaleVipAffiliate($order_info))->buy();
//                }
...
// 	        		$row[$key]['order_type_name_style'] = '<span class="ecjiafc-red">退款</span>';
    ...
    private function calculate_his_shop_percent($store_id, $amount)
    {
        //他店佣金比例
        $StoreFranchiseeModel = StoreFranchiseeModel::where('store_id', $store_id)->first();
        $his_shop_percent = $StoreFranchiseeModel->getMeta('his_shop_percent', 0);

        if(empty($his_shop_percent)) {
            $his_shop_percent = ecjia::config('his_shop_percent');
        }

        $res = $his_shop_percent / 100;

        $his_shop_amount = $res * $amount;

        return $his_shop_amount;
    }
