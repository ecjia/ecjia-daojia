            ...
            <div class="ecjia-content">
                ...
                <div class="ecjia-fr">
                    <ul class="nav hover-font">
                        <li class="active"><a class="nopjax" href="{$main_url}">{t domain="ecjia-intro"}首页{/t}</a></li>
                        <li><a href="https://ecjia.com/wiki/" target="_blank">{t domain="ecjia-intro"}帮助手册{/t}</a></li>
                        {if $member_url}<li><a class="nopjax" href="{$member_url}" target="_blank">{t domain="ecjia-intro"}会员中心{/t}</a></li>{/if}
						{if ecjia::config('merchant_join_close') eq 0}<li><a class="nopjax" href="{$merchant_url}" target="_blank">{t domain="ecjia-intro"}商家入驻{/t}</a></li>{/if}
						<li><a class="nopjax ecjia-back-green" href="{$merchant_login}" target="_blank">{t domain="ecjia-intro"}商家登录{/t}</a></li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="ecjia-container">
            <div class="ecjia-content">
                <div class="ecjia-fl wt-phone">
                    ...
                    {if $mobile_touch_url}
                        ...
                        <div class="ecjia-fl phone-tips">{t domain="ecjia-intro"}鼠标点击手机体验{/t}</div>
                    {else} 
                    <!-- 62 77 -->
                ...
                <div class="ecjia-fl wt-135">
                    <div class="ecjia-desc">
                        ...
                        <div class="two-btn wt-30">
                            {if $mobile_iphone_download}<a class="ecjia-btn icon-btn" href="{$mobile_iphone_download}" target="_blank"><i class="iphone icon"></i><span>{t domain="ecjia-intro"}iPhone端下载{/t}</span></a>{/if}
                            {if $mobile_android_download}<a class="ecjia-btn icon-btn" href="{$mobile_android_download}" target="_blank"><i class="android icon"></i><span>{t domain="ecjia-intro"}Android端下载{/t}</span></a>{/if}
                        </div>
                        <div class="ecjia-code wt-50">
                            {if $mobile_iphone_qrcode}
                            <span class="mr-20">
                                <img src="{$mobile_iphone_qrcode}" alt="" width="200" height="200">{t domain="ecjia-intro"}扫一扫，体验APP{/t}
                            </span>
                            {/if}
                            {if $touch_qrcode}
                            <span style="margin-right:32px;">
                                <img src="{$touch_qrcode}" alt="" width="200" height="200">{t domain="ecjia-intro"}扫一扫，体验微信H5界面{/t}
                            </span>
                            {/if}
                        </div>
                    </div>
            ...
            <div class="ecjia-content">
                <div class="ecjia-title wt-80">
                    <h1 class="fsize-36">{$mobile_app_name}{t domain="ecjia-intro"}简介{/t}</h1>
                    <p class="wt-30">{$mobile_app_description}</p>
                </div>

				{if $mobile_app_video}
				<div class="cly-title">{t domain="ecjia-intro"}应用视频介绍{/t}</div>
                ...
                <div class="clyimges">
                    {if $screenshots}
                    <div class="cly-title wt-30">{t domain="ecjia-intro"}界面精彩截图{/t}</div>
                    <div class="wt-30">
                    ...
                    <div class="wt-50 download">
                        <div class="two-btn wt-30">
                            {if $mobile_iphone_download}<a class="ecjia-btn icon-btn blue" href="{$mobile_iphone_download}" target="_blank"><i class="iphone icon"></i>{t domain="ecjia-intro"}iPhone端下载{/t}</a>{/if}
                            {if $mobile_android_download}<a class="ecjia-btn icon-btn green" href="{$mobile_android_download}" target="_blank"><i class="android icon"></i>{t domain="ecjia-intro"}Android端下载{/t}</a>{/if}
                        </div>
                    </div>
                </div>
            </div>
            <div class="page-footer">
				<div class="outlink">
					...
					<span class="outlink-qrcode">
                        <div class="wechat-code">
							...
							<span>{t domain="ecjia-intro"}打开微信扫一扫关注{/t}</span>
						</div>
                ...
                <p>{if $company_name}{$company_name} {t domain="ecjia-intro"}版权所有{/t}{/if} {if ecjia::config('icp_number')}&nbsp;&nbsp;<a href="http://www.miibeian.gov.cn" target="_blank"> {ecjia::config('icp_number')}</a>{/if}&nbsp;&nbsp;{$commoninfo.powered}</p>
                <p>{if $shop_address}{t domain="ecjia-intro"}地址：{/t}{$shop_address} {/if} {if $service_phone} {t domain="ecjia-intro"}咨询热线：{/t}{$service_phone}{/if}</p>
            </div>
        </div>
