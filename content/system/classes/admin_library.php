...
class admin_library extends ecjia_admin {
	...
	private function read_style_and_tpl($tpl_name, $tpl_style) {
	    $style_info = array();
	    $style_info = $this->theme->get_template_info($tpl_name, $tpl_style);

	    $tpl_style_info = array();
	    $tpl_style_info = $this->theme->read_theme_style($tpl_name, 2);
	    $tpl_style_list = '';
	    if (count($tpl_style_info) > 1) {
	        foreach ($tpl_style_info as $value) {
	            $tpl_style_list .= '<span style="cursor:pointer;" onMouseOver="javascript:onSOver(\'screenshot\', \'' . $value . '\', this);" onMouseOut="onSOut(\'screenshot\', this, \'' . $style_info['screenshot'] . '\');" onclick="javascript:setupTemplateFG(\'' . $tpl_name . '\', \'' . $value . '\', \'\');" id="templateType_' . $value . '"><img src="../ecjia/themes/' . $tpl_name . '/images/type' . $value . '_';
