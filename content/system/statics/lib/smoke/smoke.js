...
/*				ff.setAttribute('class','smoke-base'); */
	...
	build: function (e, f) {
		...
		// var h = '<div class="dialog-head"><i id="smoke-close-'+f.newid+'"  class="dialog-close fontello-icon-cancel"></i></div>';

		e = e.replace(/\n/g,'<br />');
		e = e.replace(/\r/g,'<br />');
		e = '<div class="dialog-info">'+e+'</div>';
	...
	finishbuild: function(e, f, box) {
		...
		/*======================添加关闭窗口点击事件======================*/
		 // smoke.listen(
		 // 	document.getElementById('smoke-close-'+f.newid),
		 // 	"click", 
		 // 	function () {
		 // 		smoke.destroy(f.type, f.newid);
		 // 		if (f.type === 'prompt' || f.type === 'confirm'){
