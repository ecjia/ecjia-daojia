...
/**
 * input[type="number"]全局渲染数字框自动
 */
$(function(){
	var $inputs_number = $("input[type='number']").not(".nospinner");
	if($inputs_number.length > 0) {
		$inputs_number.each(function(i){
			var $this = $(this);
			$this.attr('type','texy');
			var option = {
				value 		: $this.val() || 1,		//初始值
				min 		: $this.attr('min') || 0,		//最小值
				max 		: $this.attr('max') || 99,		//最大值
				increment 	: $this.attr('increment') || 1,		//点击的微调量
				editable 	: $this.attr('editable') || true, 	//可否输入
				disabled 	: $this.attr('disabled') || false,	//是否禁用文本
			}
			$inputs_number.eq(i).spinner(option);
		});
	}
