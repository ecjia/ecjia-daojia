    ...
    <h3 class="heading">
        ...
        <div class="pull-right list_choose{if !$full} hide{/if}">
            <select name="choose_librarys">
                ...
                <option {if $val.choose} selected="selected"{/if} value="{url path='admincp/admin_library/init' args="lib={$val.File}&full=1"}">{$val.File}{if $val.Name} - {$val.Name}{/if}</option>
                <!-- {/foreach} -->
            </select>
        </div>
    </h3>
                ...
                <div class="span3 chat_sidebar{if $full} hide{/if}">
                    ...
                    <div class="ms-selectable">
                        <div class="template_list" id="ms-custom-navigation">
                            ...
                            <ul class="unstyled">
                                <!-- {foreach from=$libraries item=val key=key} -->
                                <li class="ms-elem-selectable"><a class="data-pjax{if $val.choose} choose{/if}" href="{url path='admincp/admin_library/init' args="lib={$val.File}&full=0"}">{$val.File} <br /> {$val.Name}</a></li>
                                <!-- {/foreach} -->
                            </ul>
                        </div>
                    </div>
