 ...
 * 自动处理插件
 */
class cron_express_auto_dispatch extends CronAbstract
{
    ...
    public function run() {
        ...
        /*获取待指派订单*/
    	$wait_grab_list = RC_DB::table('express_order')->where('status', 0)->get();
    	/*自动派单*/
        if (!empty($wait_grab_list)) {
        	foreach ($wait_grab_list as $row) {
        		if ($row['shipping_code'] == 'ship_ecjia_express' && empty($row['staff_id']) && $row['status'] == '0') {
        			$params = array(
        				'express_id' => $row['express_id'],
        			);
        			$result = RC_Api::api('express', 'ecjiaauto_assign_expressOrder', $params);
        		} elseif ($row['shipping_code'] == 'ship_o2o_express' && empty($row['staff_id']) && $row['status'] == '0') {
        			$params = array(
        					'express_id' => $row['express_id'],
