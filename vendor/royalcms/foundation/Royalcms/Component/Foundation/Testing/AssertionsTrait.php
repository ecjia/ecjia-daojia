...
trait AssertionsTrait
{
    ...
    {
        PHPUnit::assertInstanceOf('Royalcms\Component\Http\RedirectResponse', $this->response);

        PHPUnit::assertEquals($this->royalcms['url']->to($uri), $this->response->headers->get('Location'));
    ...
    public function assertSessionHas($key, $value = null)
    {
        ...
        if (is_null($value)) {
            PHPUnit::assertTrue($this->royalcms['session.store']->has($key), "Session missing key: $key");
        } else {
            PHPUnit::assertEquals($value, $this->royalcms['session.store']->get($key));
        }
    }
