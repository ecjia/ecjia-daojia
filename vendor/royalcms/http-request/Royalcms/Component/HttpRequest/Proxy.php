 ...
 * through the proxy. The list should be presented in a comma separated list, wildcards using * are supported, eg. *.royalcms.cn</li>
 * </ol>
 *
 ...
 *     define('RC_PROXY_BYPASS_HOSTS', 'localhost, www.example.com, *.royalcms.cn');
 *
 * @since 2.8.0
 */
class Proxy {
	...
	public function send_through_proxy( $uri ) {
		 ...
		 */
		$result = RC_Hook::apply_filters( 'pre_http_send_through_proxy', null, $uri, $check, $home );
		if ( ! is_null( $result ) )
			return $result;
		...
		if ( null === $bypass_hosts ) {
			$bypass_hosts = preg_split('|,\s*|', RC_PROXY_BYPASS_HOSTS);

			if ( false !== strpos(RC_PROXY_BYPASS_HOSTS, '*') ) {
