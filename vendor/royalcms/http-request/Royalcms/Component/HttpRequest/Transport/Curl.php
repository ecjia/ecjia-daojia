...
class Curl {
	...
	public function request($url, $args = array()) {
		...
		if ( $is_local ) {
			...
			$ssl_verify = RC_Hook::apply_filters( 'https_local_ssl_verify', $ssl_verify );
		} elseif ( ! $is_local ) {
			/** This filter is documented in wp-includes/class-wp-http-streams.php */
			$ssl_verify = RC_Hook::apply_filters( 'https_ssl_verify', $ssl_verify );
		}
		...
		if ( $r['stream'] ) {
			if ( ! RC_Config::get('debug') )
				$this->stream_handle = @fopen( $r['filename'], 'w+' );
			else
				$this->stream_handle = fopen( $r['filename'], 'w+' );
			if ( ! $this->stream_handle )
				return RC_Error::make( 'http_request_failed', sprintf( __( 'Could not open handle for fopen() to %s' ), $r['filename'] ) );
		} else {
		 ...
		 * @param string   $url     The request URL.
		 */
		RC_Hook::do_action_ref_array( 'http_api_curl', array( &$handle, $r, $url ) );
		...
		if ( ! $r['blocking'] ) {
			...
			if ( $curl_error ) {
				curl_close( $handle );
				return RC_Error::make( 'http_request_failed', $curl_error );
			}
			...
			if ( CURLE_WRITE_ERROR /* 23 */ == $curl_error ) {
				if ( ! $this->max_body_length || $this->max_body_length != $bytes_written_total ) {
					if ( $r['stream'] ) {
						...
						return RC_Error::make( 'http_request_failed', __( 'Failed to write request to temporary file.' ) );
					} else {
						curl_close( $handle );
						return RC_Error::make( 'http_request_failed', curl_error( $handle ) );
					}
				}
			} else {
			    $curl_error = curl_error( $handle );
	...
	public static function test( $args = array() ) {
		 ...
		 */
		return RC_Hook::apply_filters( 'use_curl_transport', true, $args );
	}
}
