...
abstract class TestCase extends \PHPUnit_Framework_TestCase {
	 ...
	 *
	 * @var \Royalcms\Component\Foundation\Testing\Client
	 */
	protected $client;
	...
	protected function refreshApplication()
	{
		...
		$this->royalcms->setRequestForConsoleEnvironment();
	...
	{
		$uri = $this->royalcms['url']->action($action, $wildcards, true);
	...
	{
		$uri = $this->royalcms['url']->route($name, $routeParameters);
	...
	public function assertRedirectedTo($uri, $with = array())
	{
		...
		$this->assertInstanceOf('Royalcms\Component\HttpKernel\RedirectResponse', $response);

		$this->assertEquals($this->royalcms['url']->to($uri), $response->headers->get('Location'));
	...
	{
		$this->assertRedirectedTo($this->royalcms['url']->route($name, $parameters), $with);
	}
	...
	{
		$this->assertRedirectedTo($this->royalcms['url']->action($name, $parameters), $with);
	}
	...
	public function assertSessionHas($key, $value = null)
	{
		...
		if (is_null($value))
		{
			$this->assertTrue($this->royalcms['session.store']->has($key), "Session missing key: $key");
		}
		else
		{
			$this->assertEquals($value, $this->royalcms['session.store']->get($key));
		}
	}
	...
	public function assertSessionHasErrors($bindings = array(), $format = null)
	{
		...
		$errors = $this->royalcms['session.store']->get('errors');
	...
	{
		$this->royalcms['royalcmd']->call('db:seed', array('--class' => $class));
	}
	...
	{
		return new Client($this->royalcms, $server);
	}
