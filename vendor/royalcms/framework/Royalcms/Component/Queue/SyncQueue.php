...
use Exception;
use Throwable;
use Royalcms\Component\Queue\Jobs\SyncJob;
...
class SyncQueue extends Queue implements QueueContract
{
     ...
     *
     * @param  string  $payload
     * @return \Royalcms\Component\Queue\Jobs\SyncJob
     */
    ...
    protected function raiseAfterJobEvent(Job $job)
    {
        ...
        if ($this->container->bound('events')) {
            $this->container['events']->fire('royalcms.queue.after', ['sync', $job, $data]);
        }
    }
    ...
    protected function raiseFailedJobEvent(Job $job)
    {
        ...
        if ($this->container->bound('events')) {
            $this->container['events']->fire('royalcms.queue.failed', ['sync', $job, $data]);
        }
    }
}
