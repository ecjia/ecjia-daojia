...
class Curl
{
    ...
    function request($url, $args = array())
    {
        ...
        if ($is_local) {
             ...
             */
            $ssl_verify = Hook::apply_filters('https_local_ssl_verify', $ssl_verify);
        } elseif (! $is_local) {
            /**
             * This filter is documented in wp-includes/class-http.php
             */
            $ssl_verify = Hook::apply_filters('https_ssl_verify', $ssl_verify);
        }
        ...
        if (true === $r['blocking']) {
            curl_setopt($handle, CURLOPT_HEADERFUNCTION, array(
                $this,
                'stream_headers'
            ));
            curl_setopt($handle, CURLOPT_WRITEFUNCTION, array(
                $this,
                'stream_body'
            ));
        }
         ...
         *       
         * @param
         *            resource &$handle The cURL handle returned by curl_init().
         * @param array $r
         *            The HTTP request arguments.
         * @param string $url
         *            The destination URL.
         */
        Hook::do_action_ref_array('http_api_curl', array(
            &$handle,
            $r,
            $url
        ));
        ...
        if (! $r['blocking']) {
            ...
            if (in_array(curl_getinfo($handle, CURLINFO_HTTP_CODE), array(
                301,
                302
            ))) {
                curl_close($handle);
        ...
        $response['message'] = Status::get_status_header_desc($response['code']);
    ...
    public static function test($args = array())
    {
         ...
         * @since 3.2.0
         *       
         * @param bool $use_class
         *            Whether the class can be used. Default true.
         * @param array $args
         *            An array of request arguments.
         */
        return Hook::apply_filters('use_curl_transport', true, $args);
    }
}
