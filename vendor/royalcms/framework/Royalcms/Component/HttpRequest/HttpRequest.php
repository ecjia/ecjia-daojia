 ...
 *
 * Will eventually replace and standardize the Royalcms HTTP requests made.
 *
 * @package RC
 * @subpackage HTTP
 * @since 3.2.0
...
class HttpRequest {
    ...
    public static function remote_request($url, $args = array()) {
        $objFetchSite = self::_http_get_object();
    ...
    public static function remote_retrieve_headers( $response ) {
        if ( Error::is_error($response) || ! isset($response['headers']) || ! is_array($response['headers']))
            return array();
    ...
    public static function remote_retrieve_response_code( $response ) {
        if ( Error::is_error($response) || ! isset($response['response']) || ! is_array($response['response']))
            return '';
    ...
    public static function remote_retrieve_body( $response ) {
        if ( Error::is_error($response) || ! isset($response['body']) )
            return '';
    ...
    public static function get_http_origin() {
         ...
         */
        return Hook::apply_filters( 'http_origin', $origin );
    }
    ...
    public static function get_allowed_http_origins() {
        $admin_origin = parse_url( Uri::admin_url() );
        $home_origin = parse_url( Uri::home_url() );
         ...
         * }
        */
        return Hook::apply_filters( 'allowed_http_origins' , $allowed_origins );
    }
    ...
    public static function is_allowed_http_origin( $origin = null ) {
         ...
         */
        return Hook::apply_filters( 'allowed_http_origin', $origin, $origin_arg );
    }
    ...
    public static function http_validate_url( $url ) {
        $original_url = $url;
        $url = Kses::bad_protocol( $url, array( 'http', 'https' ) );
        ...
        $parsed_home = @parse_url( Uri::home_url() );
        ...
        if ( ! $same_host ) {
            ...
            if ( $ip ) {
                ...
                if ( 127 === $parts[0] || 10 === $parts[0]
                    ...
                    */
                    if ( ! Hook::apply_filters( 'http_request_host_is_external', false, $host, $url ) )
                        return false;
                }
            }
        }
