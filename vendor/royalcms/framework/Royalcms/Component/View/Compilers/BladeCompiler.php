...
class BladeCompiler extends Compiler implements CompilerInterface
{
    ...
    public function directiveIf($name, callable $callback)
    {
        ...
        $this->directive($name, function ($expression) use ($name) {
            return $expression
                    ? "<?php if (\Royalcms\Component\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                    : "<?php if (\Royalcms\Component\Support\Facades\Blade::check('{$name}')): ?>";
        });

        $this->directive('else'.$name, function ($expression) use ($name) {
            return $expression
                ? "<?php elseif (\Royalcms\Component\Support\Facades\Blade::check('{$name}', {$expression})): ?>"
                : "<?php elseif (\Royalcms\Component\Support\Facades\Blade::check('{$name}')): ?>";
        });
