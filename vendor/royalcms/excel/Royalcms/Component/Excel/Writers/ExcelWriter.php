<?php namespace Royalcms\Component\Excel\Writers;
...
class ExcelWriter {
    ...
    public function string($ext = 'xls')
    {
        ...
        if (!$this->writer)
            throw new ExcelException('[ERROR] No writer was set.');
    ...
    protected function _render()
    {
        ...
        if ($this->excel->getSheetCount() < 0)
            throw new ExcelException('[ERROR] Aborting spreadsheet render: no sheets were created.');
    ...
    public function getParser()
    {
        // Init the parser
        if (!$this->parser)
            $this->parser = royalcms('excel.parsers.view');
    ...
    {
        if (headers_sent()) throw new ExcelException('[ERROR]: Headers already sent');
    ...
    public function __call($method, $params)
    {
        ...
        throw new ExcelException('[ERROR] Writer method [' . $method . '] does not exist.');
    }
}
