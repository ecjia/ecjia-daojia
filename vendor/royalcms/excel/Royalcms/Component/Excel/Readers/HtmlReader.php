...
use Royalcms\Component\Excel\Classes\ExcelWorksheet;

/**
 *
 * Royalcms Excel HTML reader
 *
 */
class HtmlReader extends PHPExcel_Reader_HTML {
     ...
     * @param bool|ExcelWorksheet|null|PHPExcel $obj
    ...
    public function load($pFilename, $isString = false, $obj = false)
    {
        ...
        elseif ( $obj instanceof ExcelWorksheet )
        {
            // Load into this instance
     ...
     * @throws  PHPExcel_Reader_Exception
     */
    public function loadIntoExistingSheet($pFilename, ExcelWorksheet $sheet, $isString = false)
    {
        $isHtmlFile = false;
    ...
    {
        if ( ($columns = $sheet->getAutosize()) !== false )
        {
            if ( is_array($columns) )
            {
