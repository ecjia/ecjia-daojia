...
class AliyunOSS
{
    ...
    public function createObjectDir($key)
    {
        $dirName = rtrim($key, '/') . '/';
    ...
    public function getUrl($key, $expire_time, $options = [])
    {
     ...
     * @return
     */
    public function getObject($key, $options = [])
    {
     ...
     * @return
     */
    public function getObjectMetadata($key, $options = [])
    {
     ...
     * @return
     */
    public function deleteObject($key, $options = [])
    {
    ...
    /**
     * 批量删除阿里云中存储的文件
     * @param array $keys 存储keys（文件的路径和文件名）
     * @param bool $quiet 请求默认是详细(verbose)模式
     * @return
     */
    public function deleteMultipleObjects($keys, $quiet = false, $options = [])
    {
     ...
     * @param string $marker 用户设定结果从marker之后按字母排序的第一个开始返回。
     * @param string $maxKeys 用于限定此次返回object的最大数，如果不设定，默认为100。
     * @param string $delimiter 用于对Object名字进行分组的字符。
     * @return \Royalcms\Component\Aliyun\OSS\Models\ObjectListing
     */
    public function listObjects($prefix, $marker = null, $maxKeys = null, $delimiter = null, $options = [])
    {
    ...
    public function getAllObjectKey($bucketName, $options = [])
    {
     ...
     */
    public function getAllObjectKeyWithPrefix($bucketName, $folder_name, $nextMarker = '', $options = [])
    {
     ...
     * @return \Royalcms\Component\Aliyun\OSS\Models\CopyObjectResult
     */
    public function copyObject($sourceBuckt, $sourceKey, $destBucket, $destKey, $options = [])
    {
        if ($sourceBuckt === null) {
     ...
     * @return \Royalcms\Component\Aliyun\OSS\Models\CopyObjectResult
     */
    public function moveObject($sourceBuckt, $sourceKey, $destBucket, $destKey, $options = [])
    {
        if ($sourceBuckt === null) {
            $sourceBuckt = $this->bucket;
        }
     ...
     * 获取指定Bucket的访问权限
     * @param string $bucketName 存储容器名称
     * @return \Royalcms\Component\Aliyun\OSS\Models\AccessControlPolicy
     */
