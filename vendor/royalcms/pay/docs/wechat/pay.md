...
return $wechat->wap($order)->send(); // 框架中请直接 return $wechat->wap($order)
```
...
return $wechat->app($order)->send(); // 框架中请直接 return $wechat->app($order)
```
...
返回只会返回两种类型 `Symfony\Component\HttpFoundation\Response` 或 `Royalcms\Component\Supports\Collection`

* 返回 Response 类型时，可以通过 `return $response->send()` 直接进行返回（ 框架中使用请直接`return $response` ）
