...
class Socket
{
	...
	private function sslFilePath()
	{
	    return $pem_file = config('websocket::websocket.pem_file');
	}
	
	private function applySSLContext()
	{		
		$pem_file = $this->sslFilePath();
		$pem_passphrase = config('websocket::websocket.pem_passphrase');//shinywss	证书密码	
		...
		if (!file_exists($pem_file))
		{
			$dn = array(
				...
				"commonName" => "ecjia.com",
				"emailAddress" => "ecjia@ecjia.com"
			);
