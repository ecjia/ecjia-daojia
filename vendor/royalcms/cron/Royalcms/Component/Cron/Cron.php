...
class Cron
{
    ...
    private static function log($level, $message)
    {
        ...
        if (is_bool($royalcmsLogging) && $royalcmsLogging) {
            switch (strtolower($level)) {
                case "debug":
                    \RC_Logger::getLogger()->debug($message);
                    break;
                case "info":
                    \RC_Logger::getLogger()->info($message);
                    break;
                case "notice":
                    \RC_Logger::getLogger()->notice($message);
                    break;
                case "warning":
                    \RC_Logger::getLogger()->warning($message);
                    break;
                case "error":
                    \RC_Logger::getLogger()->error($message);
                    break;
                case "critical":
                    \RC_Logger::getLogger()->critical($message);
                    break;
                case "alert":
                    \RC_Logger::getLogger()->alert($message);
                    break;
                case "emergency":
                    \RC_Logger::getLogger()->emergency($message);
                    break;
                default:
                    throw new \InvalidArgumentException('Invalid log $level parameter with string ' . $level . '.');
            }
