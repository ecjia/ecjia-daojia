...
<form class="ecjia-user ecjia-login-user-profile-form ecjia-login-padding-top" name="user_profile">
	<div class="ecjia-form">
    	<div class="form-group ecjia-login-margin-lr ecjiaf-bt right-angle">
    		<label class="input">
    		    {if $limit_time}<p class="not_modify">{t domain="h5"}禁止修改{/t}</p>{/if}
    			<input class="ecjia-login-pa-left" id="username-modify" {if $limit_time}disabled="true"{/if}name="username" type="text" placeholder='{t domain="h5"}请输入用户名{/t}'  value="{$user.name}">
    		</label>
    	</div>
    	<p class="ecjia-margin-l ecjia-margin-t">{t domain="h5"}4-20个字符，可由中英文、数字、"_"、"-"组成{/t}</p>
