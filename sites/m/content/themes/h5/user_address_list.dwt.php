...
<div class="ecjia-address-list">
	<div class="nav-header">
		<a href="{url path='user/address/add_address' args='clear=1'}">
			<i class="icon-add-address"></i>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{t domain="h5"}新建收货地址{/t}
	...
	<section>
		...
		<ul class="ecjia-list list-one ecjia-margin-b" id="J_ItemList">
			...
			<li class="ecjia-margin-t">
				...
				<p class="ecjiaf-fl"><i class="icon-is-default"></i>&nbsp;&nbsp;{t domain="h5"}设为默认{/t}</p>
				<!-- {else} -->
				<p class="setdefault ecjiaf-csp ecjiaf-fl" data-id="{$value.id}"><i class="icon-not-default"></i>&nbsp;&nbsp;{t domain="h5"}设为默认{/t}</p>
				<!-- {/if} -->
				<a class="delete-address nopjax ecjiaf-fr ecjia-margin-l external" href="javascript:;" data-toggle="del_list" data-url="{url path='user/address/del_address'}" data-id="{$value.id}" data-msg='{t domain="h5"}您确定要删除此收货地址吗？{/t}'><div class="icon-delete-address"></div>{t domain="h5"}删除{/t}</a>
				<a class="edit-address ecjiaf-fr" href="{RC_uri::url('user/address/edit_address')}&id={$value.id}&clear=1"><div class="icon-edit-address"></div>{t domain="h5"}编辑{/t}</a>
			</li>
			<!-- {/foreach} -->
			<!-- 配送地址end-->
		</ul>
		...
		<ul class="ecjia-list list-one ecjia-margin-b over" id="J_ItemList">
			...
			<li class="ecjia-margin-t">
				...
				<p class="ecjiaf-fl"><i class="icon-is-default"></i>&nbsp;&nbsp;{t domain="h5"}设为默认{/t}</p>
				<!-- {else} -->
				<p class="ecjiaf-csp ecjiaf-fl" data-id="{$value.id}"><i class="icon-not-default"></i>&nbsp;&nbsp;{t domain="h5"}设为默认{/t}</p>
				<!-- {/if} -->
				<a class="delete-address nopjax ecjiaf-fr ecjia-margin-l external" href="javascript:;" data-toggle="del_list" data-url="{url path='user/address/del_address'}" data-id="{$value.id}" data-msg='{t domain="h5"}您确定要删除此收货地址吗？{/t}'><div class="icon-delete-address"></div>{t domain="h5"}删除{/t}</a>
				<a class="edit-address ecjiaf-fr" href="{RC_uri::url('user/address/edit_address')}&id={$value.id}&clear=1"><div class="icon-edit-address"></div>{t domain="h5"}编辑{/t}</a>
			</li>
