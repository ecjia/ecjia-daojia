...
<div class="ecjia-account-list-f">
    <div class="ecjia-account-list">
        <ul class="ecjia-list ecjia-list-three ecjia-nav ecjia-account ecjia-bonus-border-right1">
        	...
        	<li><a {if $status eq 'deposit'}class="ecjia-green right-bottom ecjia-green-rf"{else}class="right-bottom ecjia-green-rf"{/if} id="right-bottom" href="{url path='user/account/record' args='status=deposit'}">{t domain="h5"}充值{/t}</a></li>
        </ul>
    </div>
</div>
{if $status eq ''}
