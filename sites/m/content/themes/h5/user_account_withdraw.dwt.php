...
<form class="ecjia-account ecjia-form user-profile-form" action="{url path='user/account/withdraw_account'}" method="post">
	<p class="account-top text-ty">{t domain="h5"}账户余额：{/t}{$user.formated_user_money}</p>
	<div class="form-group form-group-text account-lr-fom no-border">
		<label class="input">
			<span>{t domain="h5"}金额{/t}</span>
			<input placeholder='{t domain="h5"}建议提现100元以上的金额{/t}' name="amount" value="" />
		</label>
	</div>
	<div class="account-top2">
	    <p1 class="text-ty">{t domain="h5"}备注：（最长100个字）{/t}</p1>
		...
		<div class="text-center account-top2">
			<input class="btn btn-info" name="submit" type="submit" value='{t domain="h5"}提现申请{/t}' />
		</div>
	</div>	
</form>
<!-- {/block} -->
