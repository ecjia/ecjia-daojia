...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
	<div class="form-group">
		<label class="input">
			<input name="consignee" placeholder="收货人姓名必填" type="text" value="{$consignee.consignee|escape}" datatype="s1-15" errormsg="请输入长度为3 ~ 15的收货人姓名" />
		</label>
	</div>
	<div class="form-group">
		<label class="input">
			<input name="mobile" placeholder="手机必填" type="text" value="{$consignee.mobile|escape}" datatype="m" errormsg="请输入正确格式的联系方式" />
		</label>
	</div>
	...
	<!-- 购物车中存在实体商品显示详细地址 -->
		<div class="form-group">
			<label class="textarea">
				<textarea name="address"  id="address" placeholder="详细地址" datatype="*" >{$consignee.address|escape}</textarea>
			</label>
		</div>
<!-- {/if} -->
<div class="ecjia-margin-t btn-consignee {if $smarty.session.user_id gt 0 and $consignee.address_id gt 0} two-btn {/if}">
	...
	<!-- 如果登录了，显示删除按钮 --> 
	<input  class="btn btn-info"  onclick="if (confirm('您确定要删除该收货人信息吗？')) location.href='{url path='flow/drop_consignee' args="id={$consignee.address_id}"}'" value="删除">
	<!-- {/if} -->
	<input class="btn btn-info" name="Submit" type="submit" value="配送至这个地址">
</div>
