...
class CreateStoreFranchiseeTable extends Migration {
	...
	public function up()
	{
		...
		RC_Schema::create('store_franchisee', function(Blueprint $table)
		{
		    $table->increments('store_id')->comment('店铺id');
		    $table->smallInteger('cat_id')->unsigned()->default('0')->comment('店铺分类id');
		    $table->tinyInteger('validate_type')->default('1')->comment('入驻类型');
		    $table->string('manage_mode', 10)->default('join')->index('manage_mode')->comment('经营模式，self自营，join入驻商家');
		    $table->string('merchants_name', 60)->unique('merchants_name')->comment('店铺名称');
		    $table->string('shop_keyword', 50)->nullable()->comment('店铺关键字');
		    $table->tinyInteger('status')->default('1')->index('status')->comment('店铺锁定 1正常，2锁定');
		    $table->tinyInteger('shop_close')->default('1')->index('shop_close')->comment('暂时关闭商店 0否，1是');
		    ...
		    $table->integer('confirm_time')->unsigned()->default('0')->comment('审核通过时间');
		    ...
		    $table->tinyInteger('identity_status')->default('0')->index('identity_status')->comment('入驻身份信息认证状态，0、待审核，1、审核中，2、审核通过，3、拒绝通过');
		    ...
		    $table->integer('percent_id')->unsigned()->nullable()->index('percent_id')->comment('结算比例id');
		    ...
		    $table->string('geohash', 15)->nullable()->index('geohash');
	...
	{
		RC_Schema::drop('store_franchisee');
	}
