...
class CreateFriendLinkTable extends Migration {
	...
	public function up()
	{
        if (RC_Schema::hasTable('friend_link')) {
            return ;
        }

		RC_Schema::create('friend_link', function(Blueprint $table)
		{
			$table->increments('link_id')->comment('链接名称ID');
			$table->string('link_name')->comment('链接名称');
			$table->string('link_url')->comment('链接地址');
			$table->string('link_logo')->nullable()->comment('管理操作内容');
			$table->string('link_target', 20)->nullable()->default('_blank')->comment('登录者登录IP');
			$table->tinyInteger('show_order')->unsigned()->default('50')->comment('排序');
