...
class RecreateCronsTable extends Migration {
	...
	public function up()
	{
	    RC_Schema::dropIfExists('crons');
	    
		RC_Schema::create('crons', function(Blueprint $table)
		{
			$table->increments('cron_id')->comment('自增id号');
			$table->string('cron_code', 60)->comment('该插件文件在相应路径下的不包括.php部分的文件名，运行该插件将通过该字段的值寻找将运行的文件');
			...
			$table->integer('runtime')->unsigned()->nullable()->comment('第三方个人信息');
			$table->integer('nexttime')->unsigned()->nullable()->comment('该计划任务下次被执行的时间');
			$table->tinyInteger('enabled')->unsigned()->default('1')->comment('该计划任务是否开启；0，关闭；1，开启');
			$table->tinyInteger('run_once')->unsigned()->default('0')->comment('执行后是否关闭，这个关闭的意思还得再研究下');
			$table->string('allow_ip', 255)->nullable()->comment('允许运行该计划任务的服务器ip');
