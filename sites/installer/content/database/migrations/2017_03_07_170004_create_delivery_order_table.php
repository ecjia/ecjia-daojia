...
class CreateDeliveryOrderTable extends Migration {
	...
	public function up()
	{
		...
		RC_Schema::create('delivery_order', function(Blueprint $table)
		{
		    $table->increments('delivery_id')->comment('自动增长id');
		    $table->string('delivery_sn', 20)->comment('交货编码');
		    $table->string('order_sn', 20)->comment('订单编码');
		    $table->mediumInteger('order_id')->unsigned()->default('0')->index('order_id')->comment('订单id');
		    $table->string('invoice_no', 50)->nullable()->comment('发货单号');
		    $table->integer('add_time')->unsigned()->default('0')->comment('添加时间');
		    $table->tinyInteger('shipping_id')->unsigned()->default('0')->comment('配送id');
		    $table->string('shipping_name', 120)->nullable()->comment('配送中心名称');
		    ...
		    $table->string('best_time', 120)->nullable()->comment('最佳送货时间');
		    $table->string('postscript', 255)->nullable()->comment('订单附言');
		    $table->string('how_oos', 120)->nullable()->comment('缺货处理');
		    $table->decimal('insure_fee', 10, 2)->unsigned()->default('0.00')->comment('保险费');
		    $table->decimal('shipping_fee', 10, 2)->unsigned()->default('0.00')->comment('运输费');
		    ...
		    $table->smallInteger('agency_id')->unsigned()->default('0')->comment('办事处id');
		    $table->integer('store_id')->unsigned()->default('0')->index('store_id')->comment('店铺ID');
		    
		    $table->index(array('order_id', 'store_id'), 'store_order');
		    $table->unique(array('delivery_id', 'store_id'), 'store_delivery_order');
		});
	}
