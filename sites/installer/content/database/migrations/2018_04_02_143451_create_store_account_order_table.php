...
class CreateStoreAccountOrderTable extends Migration {
	...
	public function up()
	{
		...
		RC_Schema::create('store_account_order', function(Blueprint $table)
		{
			...
			$table->decimal('amount', 10, 2)->comment('总数');
			$table->integer('admin_id')->default('0')->comment('管理员id');
			$table->string('admin_name', 60)->nullable()->comment('管理员名称');
			$table->string('admin_note', 255)->nullable()->comment('管理员备注');
			$table->string('staff_note', 255)->nullable()->comment('员工备注');
			$table->string('process_type', 20)->default('buy')->comment('charge充值，withdraw提现，bill结算');
			$table->string('pay_code', 60)->nullable()->comment('支付代码');
